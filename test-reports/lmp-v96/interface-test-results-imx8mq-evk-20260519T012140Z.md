...
- **Run:** `2026-05-19T01:30:27+00:00`
- **Device:** `imx8mq-evk-82119d6f32efaa6` @ `192.168.101.172`
...
| Failed systemd units | 0 | PASS |
| zram swap (`/dev/zram0`) | size=`2.8G` | PASS |
| zram compression | `lz4` | INFO |
| `dev-zram0.swap` unit | `active` | PASS |
| Kernel oops/BUG/stall scan (dmesg) | none this boot | PASS |
| Kernel error-level log lines (dmesg) | `6` err+ lines (INFO — trend across OTAs; some probe-defer noise is benign) | INFO |
...
| `fw_printenv` returns env | 129 vars | PASS |
| `bootcount` | `0` | PASS |
| `bootlimit` | `3` | PASS |
| `upgrade_available` | `0` | PASS |
| `bootupgrade_available` | `0` | PASS |
| `rollback` | `0` | PASS |
| `kernel_image` set | `/boot/ostree/lmp-577be93567e830217eb2f4c9349eb8329dae586…` | PASS |
...
| Check | Result | Verdict |
|---|---|---|
| `systemd` | running (pid=414) | PASS |
| `dockerd` | running (pid=771) | PASS |
| `containerd` | running (pid=637) | PASS |
| `NetworkManager` | running (pid=604) | PASS |
| `wpa_supplicant` | running (pid=624) | PASS |
| `tee-supplicant` | running (pid=525) | PASS |
| `aktualizr-lite` | running (pid=1160) | PASS |
| `fioconfig` | running (pid=646) | PASS |
...
|---|---|---|
| `soc_id` | `i.MX8MQ` | PASS |
| CAAM RNG (`/dev/hwrng`) | 8 bytes: `389bb1efffde7008` | PASS |
| VPU device nodes | ` /dev/mxc_hantro /dev/video0` | PASS |
| Vivante GPU (`/dev/galcore`) | present | PASS |
| GPU 2D/VG core | none separate — the single GC7000Lite does 2D+3D; kernel `disable 2d/vg!` at probe is expected | INFO |

## §11 USB (always-on baseline)

| Check | Result | Verdict |
|---|---|---|
| DWC3 USB-3.0 controllers bound (`dwc3`) | `2` (expect 2: `38100000.usb` `38200000.usb`) | PASS |
| xHCI host controllers bound (`xhci-hcd`) | `1` | PASS |
| USB OTG/UDC controller | `38100000.usb` | PASS |
| USB root hubs | `2` (INFO — 0 is fine if no port is in host mode) | INFO |
| USB-3.0 root hub (`1d6b:0003`) | `1` present (INFO — 0 if no host-mode port up) | INFO |
...
| `mmc1` device node (SD slot = usdhc2) | `30b50000.mmc` (expect `30b50000.mmc`) | INFO |
...
| eMMC boot device | `/dev/mmcblk0` (non-removable, `device/type`=MMC) | PASS |
...
| `hwclock --show` | `2026-05-19 01:32:33.139804+00:00` | PASS |
...
| Check | Result | Verdict |
|---|---|---|
| `MemTotal` | `2957640` kB (≥2,700,000 — 3 GiB board, expect ~2,957,640) | PASS |
| `CmaTotal` | `917504` kB (expect 917504 — DT `linux,cma` 0x38000000 / 896 MiB) | INFO |
| `MemAvailable/MemTotal` | `0.810` (>0.5) | PASS |
...
| Check | Result | Verdict |
|---|---|---|
| Thermal zones | `1` (expect 1: `cpu-thermal`) | INFO |
| `thermal_zone0` (`cpu-thermal`) | `49.0°C` | INFO |
| `thermal_zone0` temp in 15-85°C | `49.0°C` | PASS |
| `thermal_zone0` trip points | passive=`85°C` critical=`95°C` (expect 85 / 95) | INFO |
...
| Display connector (`card0-HDMI-A-1`, expect `HDMI-A-1`) | `connected`, mode `1920x1080`, fb0 `1920,1080` | PASS |
| 3D GPU render (`weston-simple-egl`, GLES on GC7000Lite) | EGL/GLES context created + rendered ~5 s, no crash | PASS |

## §19 Wi-Fi radio

| Check | Result | Verdict |
|---|---|---|
| Wi-Fi station interface | `wlp1s0` (expect `wlp1s0`; M.2-card-dependent) | PASS |
| Wi-Fi driver module | `brcmfmac_wcc,brcmfmac,brcmutil` (expect `brcmfmac`; M.2-card-dependent) | PASS |
| `wpa_supplicant` service | `active` | PASS |
| `nmcli radio wifi` | `enabled` | PASS |
| Wi-Fi scan (AP count) | `12` APs — 2.4GHz:`8` 5GHz:`9` | PASS |
...
| BT driver module | `btnxpuart` (expect `btnxpuart` — UART-attached NXP BT) | PASS |
...
| BT scan (devices discovered) | `0` — FAIL unless bench is RF-isolated / nothing advertising | FAIL |
...
| Check | Result | Verdict |
|---|---|---|
| ALSA sound cards (`/proc/asound/cards`) | `4` (expect 4) | PASS |
| ALSA card names | `imxspdif        imxaudiohdmi    imxhdmiarc      wm8524audio` (expect `imxspdif imxhdmiarc imxaudiohdmi wm8524audio`) | INFO |
| Playback devices (`aplay -l`) | `3` device(s) | PASS |
| Capture devices (`arecord -l`) | `2` device(s) | PASS |
| `/dev/snd` device nodes | `9` control/pcm nodes | PASS |
| Deferred-probe audio nodes | optional audio-expansion / BT-SCO codec nodes may stay unbound — expected on a bare EVK | INFO |
...
| Check | Result | Verdict |
|---|---|---|
| PCIe controller node(s) in devicetree | `33800000.pcie,33c00000.pcie` | INFO |
| PCIe host bridge(s) bound (`imx6q-pcie`) | `33800000.pcie,33c00000.pcie` — link is up | INFO |
| PCI devices on the bus (`/sys/bus/pci/devices`) | `3` — host bridge + endpoint(s) | PASS |
| PCIe endpoint-card link + traffic test | skipped (optional — needs a card in a PCIe/M.2 slot) | SKIP |
...
**Overall: FAIL** — 1 row(s) failed. Investigate before sign-off.

...
- **Run:** `2026-05-19T01:21:41+00:00`
- **Device:** `imx8mm-lpddr4-evk-24214209dab44e2a` @ `192.168.101.164`
...
| Kernel error-level log lines (dmesg) | `12` err+ lines (INFO — trend across OTAs; some probe-defer noise is benign) | INFO |
...
| `fw_printenv` returns env | 131 vars | PASS |
| `bootcount` | `0` | PASS |
| `bootlimit` | `3` | PASS |
| `upgrade_available` | `0` | PASS |
| `bootupgrade_available` | `0` | PASS |
| `rollback` | `0` | PASS |
| `kernel_image` set | `/boot/ostree/lmp-4295462882c60bfa3a48590b465aa0ba0fcb3df…` | PASS |
...
| `dockerd` | running (pid=729) | PASS |
| `containerd` | running (pid=594) | PASS |
| `NetworkManager` | running (pid=571) | PASS |
| `wpa_supplicant` | running (pid=695) | PASS |
| `tee-supplicant` | running (pid=502) | PASS |
| `aktualizr-lite` | running (pid=1559) | PASS |
| `fioconfig` | running (pid=612) | PASS |
...
|---|---|---|
| `soc_id` | `i.MX8MM` | PASS |
| CAAM RNG (`/dev/hwrng`) | 8 bytes: `ed69ff82ec792393` | PASS |
| VPU device nodes | ` /dev/mxc_hantro /dev/mxc_hantro_h1 /dev/video0` | PASS |
| Vivante GPU (`/dev/galcore`) | present | PASS |
| G2D 2D engine (GC320) | `libg2d` present but no g2d sample tool to exercise it; not run | SKIP |
...
| `hwclock --show` | `2026-05-19 01:24:04.783080+00:00` | PASS |
...
| Check | Result | Verdict |
|---|---|---|
| `MemTotal` | `1930644` kB (≥1,800,000 — 2 GiB board) | PASS |
| `CmaTotal` | `655360` kB (640 MiB) | PASS |
| `MemAvailable/MemTotal` | `0.819` (>0.5) | PASS |
| OOM events (this boot) | 0 | PASS |
| `Dirty` pages | `44` kB (<100,000) | PASS |
...
| Check | Result | Verdict |
|---|---|---|
| Thermal zones | `1` (imx8mm TMU = single `cpu-thermal` sensor) | PASS |
| `thermal_zone0` (`cpu-thermal`) | `55.0°C` | INFO |
| `thermal_zone0` temp in 15-85°C | `55.0°C` | PASS |
| `thermal_zone0` trip points | passive=`85°C` critical=`95°C` (⚠️ verify against device tree) | INFO |
...
| Wi-Fi driver module (QCA `ath*`) | `ath10k_sdio,ath10k_core,ath` | PASS |
| `wpa_supplicant` service | `active` | PASS |
| `nmcli radio wifi` | `enabled` | PASS |
| Wi-Fi scan (AP count) | `17` APs — 2.4GHz:`15` 5GHz:`12` | PASS |
...
| BT driver module | `hci_uart,btqca,btbcm` | PASS |
| rfkill (`hci0` not blocked) | soft-blocked=`no` | PASS |
| Controller enumerated (`bluetoothctl show`) | Manufacturer `0x001d (29)` | PASS |
| Controller power-on | `hci0 UP RUNNING` | PASS |
bash: line 411:  3415 Done                    echo fio
      3416 Segmentation fault      | sudo -S -p '' bluetoothctl --timeout 15 scan on > /dev/null 2>&1
| BT scan (devices discovered) | `2` device(s) | PASS |
...
| Check | Result | Verdict |
|---|---|---|
| ALSA sound cards (`/proc/asound/cards`) | `3` (≥3: imxspdif, wm8524, micfil) | PASS |
| On-board codec card (`wm8524` DAC) | registered | PASS |
| Playback devices (`aplay -l`) | `2` device(s) | PASS |
| Capture devices (`arecord -l`) | `2` device(s) | PASS |
| `/dev/snd` device nodes | `7` control/pcm nodes | PASS |
| Deferred-probe audio nodes | sound-ak4458 / sound-bt-sco stay unbound — expected (optional audio board / BT-SCO codec absent) | INFO |
...
| PCI devices on the bus (`/sys/bus/pci/devices`) | `1` — host bridge only, no endpoint card | INFO |

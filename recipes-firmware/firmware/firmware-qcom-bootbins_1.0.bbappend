...
SRC_URI:append:qcs615 = " \
    https://artifacts.codelinaro.org/artifactory/codelinaro-le/Qualcomm_Linux/QCS615/cdt/ADP_AIR_SA6155P_V1.zip;downloadfilename=cdt-adp-air-sa6155p_${PV}.zip;name=adp-air-sa6155p \
"
...
SRC_URI[adp-air-sa6155p.sha256sum] = "a5a98cbe1c2caa0e35d2fe0a78d56c36ea3e61a43cff9ce98c2567887454bb7f"
...
# CDT for ADP AIR SA6155P has a different filename format
do_deploy:append:qcs615() {
	find "${WORKDIR}" -maxdepth 2 -name 'adp_*bin' -exec install -m 0644 {} ${DEPLOYDIR} \;
}
do_deploy:append() {

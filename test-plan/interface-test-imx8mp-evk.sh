#!/bin/bash
# ============================================================================
# Interface test for the imx8mp-evk — device-side script.
# Runs all "always-on baseline" rows from fio-test-plan-imx8mp-evk.md
...
#       -o LogLevel=ERROR fio@<DEVICE_IP> 'bash -s' < interface-test-imx8mp-evk.sh \
...
[ "$si" = "i.MX8MP" ] && { row "\`soc_id\`" "\`$si\`" "PASS"; P; } || { row "\`soc_id\`" "\`$si\`" "FAIL"; F; }
hb=$(SUDO head -c8 /dev/hwrng 2>/dev/null | xxd -p)
[ -n "$hb" ] && { row "CAAM RNG (\`/dev/hwrng\`)" "8 bytes: \`$hb\`" "PASS"; P; } || { row "CAAM RNG" "no read" "FAIL"; F; }
vp=""; for d in /dev/mxc_hantro /dev/mxc_hantro_vc8000e; do [ -e "$d" ] && vp="$vp $d"; done
...
# G2D 2D-engine check — the i.MX 8M Plus has a dedicated Vivante 2D core (GC520L). Exercise it via
# NXP's G2D API *if* the tooling is in the image. Conditional: needs libg2d + a g2d sample tool
# + /dev/galcore — LmP images frequently ship none, so SKIP is the common, expected path.
g2dlib=$(ls /usr/lib/libg2d.so* 2>/dev/null | head -1)
g2dbin=""
for t in g2d_test g2d_overlay_test g2d_multiblit_test; do
  for c in "$t" "/opt/imx-g2d-samples/$t"; do
    command -v "$c" >/dev/null 2>&1 && { g2dbin=$c; break 2; }
  done
done
if [ -z "$g2dlib" ]; then
  row "G2D 2D engine (GC520L)" "no \`libg2d\` in image — G2D not enabled in this build; not run" "SKIP"; S
elif [ ! -e /dev/galcore ]; then
  row "G2D 2D engine (GC520L)" "\`/dev/galcore\` absent — GPU stack not up; not run" "SKIP"; S
elif [ -z "$g2dbin" ]; then
  row "G2D 2D engine (GC520L)" "\`libg2d\` present but no g2d sample tool to exercise it; not run" "SKIP"; S
else
  g2dout=$(SUDO timeout -k 2 25 "$g2dbin"); rc=$?
  if [ "$rc" = 0 ]; then
    row "G2D 2D blit (\`$g2dbin\`)" "ran clean — dedicated 2D core (GC520L) exercised via the G2D API" "PASS"; P
  else
    row "G2D 2D blit (\`$g2dbin\`)" "\`$g2dbin\` exited rc=\`$rc\`: \`$(printf %s "$g2dout" | tr '\n|' '  ' | tr -d '\r' | tail -c 110)\`" "FAIL"; F
  fi
fi

header "§11 USB host (always-on baseline)"
rh=$(lsusb | grep -c "Linux Foundation.*root hub")
[ "$rh" -ge 2 ] && { row "USB root hubs" "\`$rh\` (≥2)" "PASS"; P; } || { row "USB root hubs" "\`$rh\`" "FAIL"; F; }
# count bound xHCI host controllers from sysfs (persistent — dmesg ring buffer rotates)
xc=$(ls /sys/bus/platform/drivers/xhci-hcd/ 2>/dev/null | grep -vE '^(bind|unbind|uevent|module)$' | wc -l)
[ "$xc" -ge 1 ] && { row "xHCI host controllers bound" "\`$xc\`" "PASS"; P; } || { row "xHCI host controllers" "\`$xc\`" "FAIL"; F; }
ud=$(ls /sys/class/udc/ 2>/dev/null | head -1)
[ -n "$ud" ] && { row "USB OTG controller" "\`$ud\`" "PASS"; P; } || { row "USB OTG" "absent" "FAIL"; F; }
...
md=$(readlink -f /sys/class/mmc_host/mmc1 2>/dev/null | grep -oE '[0-9a-f]+\.mmc' | head -1)
[ "$md" = "30b50000.mmc" ] && { row "\`mmc1\` bound to \`30b50000.mmc\`" "yes" "PASS"; P; } || { row "\`mmc1\` device" "\`$md\`" "FAIL"; F; }
# card-detect GPIO: from the current-boot kernel journal (full retention, unlike the dmesg ring buffer)
cg=$(SUDO journalctl -k -b 0 2>/dev/null | grep -c "30b50000.mmc.*Got CD GPIO")
[ "$cg" -ge 1 ] && { row "Card-detect GPIO" "wired (\`Got CD GPIO\`)" "PASS"; P; } || { row "Card-detect GPIO" "no journal trace" "FAIL"; F; }
...
[ "$mt" -ge 5500000 ] && { row "\`MemTotal\`" "\`$mt\` kB (≥5,500,000)" "PASS"; P; } || { row "\`MemTotal\`" "\`$mt\` kB" "FAIL"; F; }
cma=$(awk '/^CmaTotal:/{print $2}' /proc/meminfo)
[ "$cma" = "983040" ] && { row "\`CmaTotal\`" "\`$cma\` kB" "PASS"; P; } || { row "\`CmaTotal\`" "\`$cma\` kB (expected 983040)" "FAIL"; F; }
...
[ "$zc" -eq 2 ] && { row "Thermal zones" "\`$zc\`" "PASS"; P; } || { row "Thermal zones" "\`$zc\`" "FAIL"; F; }
...
t1=$(awk '{printf "%.1f", $1/1000}' /sys/class/thermal/thermal_zone1/temp 2>/dev/null)
t1t=$(cat /sys/class/thermal/thermal_zone1/type 2>/dev/null)
row "\`thermal_zone0\` (\`$t0t\`)" "\`${t0}°C\`" "INFO"; I
row "\`thermal_zone1\` (\`$t1t\`)" "\`${t1}°C\`" "INFO"; I
for tz in thermal_zone0 thermal_zone1; do
  T=$(awk '{printf "%.1f", $1/1000}' /sys/class/thermal/$tz/temp 2>/dev/null)
  awk -v t="$T" 'BEGIN{exit !(t>=15 && t<=80)}' && { row "\`$tz\` temp in 15-80°C" "\`${T}°C\`" "PASS"; P; } || { row "\`$tz\` temp" "\`${T}°C\` out of range" "FAIL"; F; }
done
diff=$(awk -v a="$t0" -v b="$t1" 'BEGIN{d=a-b; if(d<0)d=-d; printf "%.1f",d}')
awk -v d="$diff" 'BEGIN{exit !(d<=15)}' && { row "\`|cpu-soc|\` divergence" "\`${diff}°C\` (≤15)" "PASS"; P; } || { row "Zone divergence" "\`${diff}°C\`" "FAIL"; F; }
for tz in thermal_zone0 thermal_zone1; do
  pa=$(awk '{printf "%.0f", $1/1000}' /sys/class/thermal/$tz/trip_point_0_temp 2>/dev/null)
  cr=$(awk '{printf "%.0f", $1/1000}' /sys/class/thermal/$tz/trip_point_1_temp 2>/dev/null)
  { [ "$pa" = "85" ] && [ "$cr" = "95" ]; } && { row "\`$tz\` trip points" "passive=\`${pa}°C\` critical=\`${cr}°C\`" "PASS"; P; } || { row "\`$tz\` trip points" "passive=\`$pa\` critical=\`$cr\`" "FAIL"; F; }
done
...
if systemctl list-unit-files weston.service >/dev/null 2>&1 && [ -e /dev/galcore ]; then
  ...
  # display-attached: INFO unless a monitor is connected
  hs=$(cat /sys/class/drm/card0-HDMI-A-1/status 2>/dev/null)
  if [ "$hs" = "connected" ]; then
    hm=$(head -1 /sys/class/drm/card0-HDMI-A-1/modes 2>/dev/null)
    fb=$(cat /sys/class/graphics/fb0/virtual_size 2>/dev/null)
    row "HDMI connector" "\`connected\`, mode \`$hm\`, fb0 \`$fb\`" "PASS"; P
  else
    row "HDMI connector" "\`${hs:-unknown}\` (INFO — headless bench is fine)" "INFO"; I
  fi
  ...
  if ! command -v weston-simple-egl >/dev/null 2>&1; then
    ...
    if [ -z "$xrd" ] || [ -z "$wdpy" ]; then
      ...
      if [ "$rc" = 124 ] || [ "$rc" = 137 ]; then
        row "3D GPU render (\`weston-simple-egl\`, GLES)" "EGL/GLES context created + rendered ~5 s, no crash" "PASS"; P
      else
...
wlif=$(ip -br link show 2>/dev/null | awk '/^wl/{print $1; exit}')
if [ -n "$wlif" ]; then
  row "Wi-Fi station interface" "\`$wlif\`" "PASS"; P
  drv=$(lsmod 2>/dev/null | awk '/^(moal|brcmfmac|rtl)/{print $1}' | paste -sd, -)
  [ -n "$drv" ] && { row "Wi-Fi driver module" "\`$drv\`" "PASS"; P; } || { row "Wi-Fi driver module" "none loaded" "FAIL"; F; }
...
if echo fio | sudo -S -p '' hciconfig hci0 >/dev/null 2>&1; then
  ...
  bdrv=$(lsmod 2>/dev/null | awk '/^(btnxpuart|btrtl|btbcm|hci_uart)/{print $1}' | paste -sd, -)
  [ -n "$bdrv" ] && { row "BT driver module" "\`$bdrv\`" "PASS"; P; } || { row "BT driver module" "none loaded" "FAIL"; F; }
...
header "§21 Audio"
# imx8mp-evk ALSA — working cards: imxaudioxcvr (SPDIF/eARC transceiver), wm8960audio
# (on-board WM8960 codec), audiohdmi (HDMI audio), micfilaudio (PDM mic). sound-bt-sco
# stays in deferred-probe — expected absence (BT-SCO codec link) on a bare EVK.
acards=$(grep -cE '^ *[0-9]+ \[' /proc/asound/cards 2>/dev/null)
[ "${acards:-0}" -ge 4 ] && { row "ALSA sound cards (\`/proc/asound/cards\`)" "\`$acards\` (≥4: xcvr, wm8960, hdmi, micfil)" "PASS"; P; } || { row "ALSA sound cards" "\`${acards:-0}\` (expected ≥4)" "FAIL"; F; }
grep -q wm8960 /proc/asound/cards 2>/dev/null && { row "On-board codec card (\`wm8960\`)" "registered" "PASS"; P; } || { row "On-board codec (\`wm8960\`)" "not registered" "FAIL"; F; }
grep -qi hdmi /proc/asound/cards 2>/dev/null && { row "HDMI audio card" "registered" "PASS"; P; } || { row "HDMI audio card" "not registered" "FAIL"; F; }
...
row "Deferred-probe audio nodes" "sound-bt-sco stays unbound — expected (BT-SCO codec absent on a bare EVK)" "INFO"; I
row "Tone-playback test" "skipped (optional — needs a speaker / HDMI sink)" "SKIP"; S

header "§22 PCIe"
# imx8mp has one PCIe Gen3 controller (DesignWare-based, driver imx6q-pcie). A PCIe link comes
# up only with an endpoint card fitted in the slot; on a bare bench the controller often stays
...
[ -n "$pciebound" ] && { row "PCIe host bridge bound (\`imx6q-pcie\`)" "\`$pciebound\` — link is up" "INFO"; I; } || { row "PCIe host bridge bound" "not bound — no link (expected with no endpoint card)" "INFO"; I; }
...
row "PCIe endpoint-card link + traffic test" "skipped (optional — needs a card in the PCIe/M.2 slot)" "SKIP"; S

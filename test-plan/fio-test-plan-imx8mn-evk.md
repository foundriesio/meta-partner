...
Factory: `nxp-imx` · Device: `imx8mn-evk-dev` · Target machine: `imx8mn-ddr4-evk`

> ## i.MX 8M Nano EVK — board characteristics
>
> Key hardware facts this plan depends on:
>
> - **Display:** the i.MX 8M Nano has no HDMI controller — like the i.MX 8M Mini, the imx8mn-evk
>   has only a MIPI-DSI interface (`32e10000.dsi_controller`, driver `imx_sec_dsim_drv`). HDMI
>   output requires the **optional external ADV7535 DSI→HDMI bridge board** plugged into the DSI
>   MIPI connector — it is not an on-board part. The devicetree carries the `adv7535@3d` node
>   unconditionally, so when the bridge board is **not connected** the kernel logs `failed to
>   attach bridge … dsi_controller` / `adv7511 1-003d: Probe failed` and no `card0-HDMI-A-1`
>   connector appears — **this is expected when the ADV7535 add-on is absent, not a fault**; the
>   `imx-drm` subsystem still binds the DSI controller. (On the target-10 bench the ADV7535 was
>   not connected to the imx8mn-evk.)
> - **Companion core:** Cortex-M7 (the i.MX 8M Nano has a Cortex-M7, not the Mini's Cortex-M4).
> - **No NPU**; GPU is a single Vivante 3D core at `38000000.gpu` (driver `galcore` 6.4.11,
>   `/dev/galcore`). The 2D/VG core is disabled (`galcore: clk_get 2d core clock failed,
>   disable 2d/vg`) — so, unlike the Mini, there is no separate GC320 2D block.
> - **Memory:** this is the **DDR4** EVK variant — the imx8mn-ddr4-evk has **2 GiB DDR4**
>   (`MemTotal` ~1,929,656 kB after firmware reservations), a 640 MiB CMA pool
>   (`CmaTotal` 655,360 kB), and ~1.8 GiB zram swap (`SwapTotal` ~1,884,428 kB).
> - **SDP USB ID:** `1fc9:013e` — enumerates as product "SE Blank 815"; the i.MX 8M Nano uses
>   the newer single-stage `SDPS:` flow, not the Mini's two-stage `SDP:`+`SDPV:` flow. Note this
>   `1fc9:013e` ID is **shared with the i.MX 8M Plus** — both SDPS-mode i.MX8M parts present it,
>   and `uuu` groups them under chip code `MX815`. So the ID alone does not distinguish Nano from
>   Plus; the chip's USB **serial number** is its unique die ID (cross-check it against the
>   booted hostname suffix if in doubt).
> - **Wi-Fi/BT:** a split combo — Wi-Fi is a **Broadcom BCM4345/6 (CYW43455)** on SDIO (`mmc0`),
>   driver `brcmfmac`, interface `wlan0`; Bluetooth is driven by **`btnxpuart`** (NXP UART BT),
>   controller `hci0` on a UART bus. (Not the Mini's QCA9377 / `ath10k` / `btqca`.) Note the BT
>   link is not fully clean on the target-10 build — the kernel logs `hci0: Setting baudrate
>   failed (-56)` / `Setting Power Save mode failed (-22)`.
> - **uuu flash needs four artifacts** — the i.MX 8M Nano does **not** use a `sit.bin`
>   (the boot-image offset is defined in fuses).
> - **Firmware update path:** the i.MX 8M Nano uses the *alternative* (backup/restore, no
>   trial-boot) firmware-update path — there is no secondary firmware slot.
> - **Boot mode** is set via `SW1101`/`SW1102` per the EVK silkscreen (§0.1); the **`DEBUG`
>   microUSB** connector carries the A53 console (§0.2).
...
## Boot chain (imx8mn-evk, FoundriesFactory LmP)
...
| 2. **SPL** + DDR firmware | `imx-boot-<machine>` (e.g. `imx-boot-imx8mn-ddr4-evk`); flashed by uuu into eMMC partitions `bootloader` (slot A) and `bootloader_s` (slot B) | BootROM (AHAB signature) | SRK fuses |
| 3. **fit-image** — ATF (BL31) + OP-TEE (BL32) + U-Boot proper (BL33) + boot script | published as a *separate* artifact `u-boot-<machine>.itb` (e.g. `u-boot-imx8mn-ddr4-evk.itb`); flashed by uuu into eMMC partitions `bootloader2` (slot A) and `bootloader2_s` (slot B). The i.MX 8M Nano does **not** use a `sit.bin` — its boot-image offset is defined in fuses, not in a Secondary Image Table. | LmP-patched SPL via FIT signature check | **Factory-specific public key embedded in SPL's devicetree at build time** |
...
> **i.MX 8M Nano firmware-update path.** The i.MX 8M Nano uses the LmP *alternative*
> (backup/restore) boot-firmware update path — **no firmware trial-boot, no secondary firmware
> slot**: a new `imx-boot`/fit-image is applied by backing up the in-place image and writing the
> new one, with no SPL-side A/B fall-back of the firmware itself. This is unlike the i.MX 8M
> Mini's regular A/B trial-boot scheme. The OSTree-deployment rollback (`bootcount`/`bootlimit`
> →`fiovb.rollback`) described under [Mechanism under test](#mechanism-under-test-rollback) is
> unaffected — only the *firmware*-slot semantics differ. The two bullets below describe the
> regular path; the i.MX 8M Nano instead uses the *alternative* path — no secondary firmware
> slot, backup/restore.
...
**When Claude executes this plan, it should call `AskUserQuestion` at the start to collect all variables below** rather than hardcoding values. Suggested defaults reflect a typical lab bench but the plan should work for any Factory whose i.MX 8M Nano EVK target uses the `imx8mn-ddr4-evk` MACHINE.

| Variable | Suggested default | Notes |
|---|---|---|
| `$FACTORY` | `nxp-imx` | the Factory the device registers to |
| `$DEVICE` | `imx8mn-evk-dev` | a fresh device name; bump on re-runs (registration is one-shot) |
...
| `$SERIAL_DEV` | `/dev/ttyUSB1` | A53 debug UART — **confirmed on this bench**. The imx8mn-evk's debug bridge is a dual-UART (2 ttyUSB devices): `ttyUSB0` is silent (M7/secondary), `ttyUSB1` carries the A53 Linux console. |
...
- imx8mn-evk hardware on the bench, powered via USB-C PD (the `8MMINI-BB` base board has no barrel jack), Ethernet plugged into the lab LAN, debug microUSB cable (to the `DEBUG` connector — see §0.2) to the host
- USB-C cable available for the `Download` port (for uuu flashing in §0)
- Boot-mode switch (`SW1101`/`SW1102`) positions known — read off the EVK silkscreen (see §0.1)
...
## Phase 0 — Lab setup (imx8mn-evk)

### 0.1 Boot-mode switch cheat sheet

The imx8mn-evk selects its boot device with two boot-mode DIP switches, **`SW1101`** and
**`SW1102`**. **The board silkscreen — printed on the PCB next to those switches — documents the
switch settings for each boot source.** Read the positions for the three modes below directly off
that silkscreen (and cross-check the i.MX 8M Nano EVK hardware user guide if the silkscreen is
ambiguous); the exact bit patterns are board-revision-specific, so the silkscreen on *your* board
is the authoritative source.

| Mode | Switches | When to use |
|---|---|---|
| eMMC (default for this plan) | `SW1101` / `SW1102` per silkscreen "eMMC" | all OTA / rollback / app phases |
| USB — SDP (Serial Download Protocol) | `SW1101` / `SW1102` per silkscreen "Serial Downloader" / "USB" | first-time provisioning (§0.4) and uuu recovery |
| microSD | `SW1101` / `SW1102` per silkscreen "SD" / "MicroSD" | only if deliberately testing an SD-card boot variant |

Always power the board off before changing `SW1101`/`SW1102`. Record the current setting before changing — restoring it is a per-test ritual.

**Diagnostic clue — hostname suffix:** if the booted system reports its hostname as `<machine>-sd` (e.g. `imx8mn-ddr4-evk-sd`) instead of `<machine>` (e.g. `imx8mn-ddr4-evk`), the board booted from SD, *not* eMMC. The `-sd` suffix is not a build-flavor naming convention — it's a literal indicator that the SD-card image is what loaded. If you just flashed eMMC and you're seeing `-sd` in the hostname or login banner, `SW1101`/`SW1102` are wrong: power off, set them to the eMMC position (per the EVK silkscreen — see above), power on. (The `-sd` can read like an LmP build-variant suffix — it isn't; it's a literal boot-source tag.)

### 0.2 Cabling

Board orientation for the descriptions below: hold the EVK so the **SoM text reads normally**.

All three connectors are on the `8MMINI-BB` base board (the i.MX 8M Nano DDR4 EVK shares the i.MX 8M Mini EVK's base board), each silkscreen-labelled:

| Cable | Board port (silkscreen label) | Host side | Purpose |
|---|---|---|---|
| USB debug | **microUSB**, labelled **`DEBUG`** — bottom-left corner of the EVK, immediately left of the `Download` port | USB-A→microUSB cable; enumerates as **2×** `/dev/ttyUSB[0-1]` (dual-UART bridge) | **A53 Linux console on `ttyUSB1`** (confirmed); `ttyUSB0` is the secondary/M7 UART |
| USB-C OTG | **USB-C**, labelled **`Download`** — immediately right of the `DEBUG` connector | host USB-C / Type-A | uuu downloads in SDP mode |
| Ethernet | RJ-45 (`end0`) | lab switch | DHCP for `$DEVICE_IP` and SSH/OTA |
| Power | **USB-C**, labelled **`POWER`** (USB-C PD — the 8M-Mini-family baseboard has no barrel jack) | 5 V / 3 A | required even with the `Download` port attached |
...
screen -L -Logfile "$SERIAL_LOG" /dev/ttyUSB1 115200
...
# Four separate artifacts — all required (the imx8mn flash does NOT include a sit.bin):
fioctl targets artifacts -f $FACTORY $TARGET \
  imx8mn-ddr4-evk/lmp-factory-image-imx8mn-ddr4-evk.wic.gz > lmp-factory-image-imx8mn-ddr4-evk.wic.gz
fioctl targets artifacts -f $FACTORY $TARGET \
  imx8mn-ddr4-evk/imx-boot-imx8mn-ddr4-evk > imx-boot-imx8mn-ddr4-evk
fioctl targets artifacts -f $FACTORY $TARGET \
  imx8mn-ddr4-evk/u-boot-imx8mn-ddr4-evk.itb > u-boot-imx8mn-ddr4-evk.itb
fioctl targets artifacts -f $FACTORY $TARGET \
  imx8mn-ddr4-evk-mfgtools/mfgtool-files-imx8mn-ddr4-evk.tar.gz > mfgtool-files.tar.gz

tar xzf mfgtool-files.tar.gz              # creates mfgtool-files-imx8mn-ddr4-evk/ with uuu, full_image.uuu, bundled mfgtool SPL
# do NOT gunzip the wic — uuu's `flash -raw2sparse` reads .wic.gz directly
```

Two non-obvious things to know about these artifacts:

- **The wic is `lmp-factory-image-…`** — FoundriesFactory CI on i.MX targets publishes the factory-image variant (which includes Foundries-specific extras), not `lmp-base-console-image-…`. Adjust the artifact name if your Factory's CI publishes a different variant; check via `fioctl targets artifacts -f $FACTORY $TARGET | grep '^imx8mn-ddr4-evk/' | grep wic`.
- **The `mfgtool-files` tarball is published under a *separate* build run** named `<machine>-mfgtools/`, *not* under the main machine path. So the mfg artifact path is `imx8mn-ddr4-evk-mfgtools/mfgtool-files-imx8mn-ddr4-evk.tar.gz` — note the `-mfgtools` suffix on the run name. uuu, `full_image.uuu`, and the bootstrap mfgtool SPL (`imx-boot-mfgtool`) all live inside this tarball. Unlike the i.MX 8M Mini, the i.MX 8M Nano's single-stage `SDPS:` flow does **not** need a bootstrap fit-image (`u-boot-mfgtool.itb`).

What `full_image.uuu` actually does:

```
SDPS: boot -f imx-boot-mfgtool                        # SDPS single-stage: loads the bundled mfgtool SPL + DDR fw, then transitions straight to fastboot
FB: ucmd setenv fastboot_dev mmc
FB: ucmd mmc dev ${emmc_dev} 1; mmc erase 0 0x2000    # clear eMMC boot partition 1
FB: flash -raw2sparse all ../lmp-factory-image-imx8mn-ddr4-evk.wic.gz/*
FB: flash bootloader     ../imx-boot-imx8mn-ddr4-evk      # SPL → eMMC "bootloader" partition (slot A)
FB: flash bootloader2    ../u-boot-imx8mn-ddr4-evk.itb    # fit-image → "bootloader2" partition (slot A)
FB: flash bootloader_s   ../imx-boot-imx8mn-ddr4-evk      # SPL → "bootloader_s" (slot B, redundant)
FB: flash bootloader2_s  ../u-boot-imx8mn-ddr4-evk.itb    # fit-image → "bootloader2_s" (slot B)
# No sit.bin for imx8mn: the boot-image offset is defined in fuses
FB: ucmd mmc partconf ${emmc_dev} ${emmc_ack} 1 0
FB: done
```

Maps directly to the boot architecture: SPL goes to `bootloader{,_s}`, fit-image goes to `bootloader2{,_s}`, two slots each so fiovb has somewhere to fall back to. Unlike the i.MX 8M Mini, the i.MX 8M Nano flash writes **no `sit.bin`** — the BootROM finds the bootloader from a fuse-defined offset, so only **four** artifacts are pulled. The SDP flow is the newer single-stage `SDPS:` — `full_image.uuu` begins `SDPS: boot -f imx-boot-mfgtool`.

Power off → set `SW1101`/`SW1102` to the USB-SDP position (per the EVK silkscreen — see §0.1) → power on → connect a USB-C cable from the host to the **`Download`** port. Confirm the host sees the SDP device:

```bash
lsusb | grep -iE 'NXP|Freescale'
# Expected: "1fc9:013e NXP Semiconductors SE Blank 815" — the i.MX 8M Nano SDPS USB ID.
# (This 1fc9:013e ID is shared with the i.MX 8M Plus — both are SDPS-mode i.MX8M parts.)
#
# Better — confirm uuu detects it and read the chip serial (its unique die ID):
#   mfgtool-files-imx8mn-ddr4-evk/uuu -lsusb
# Expect a row: "Chip MX815  Pro SDPS:  Vid 0x1FC9  Pid 0x013E  Serial_no <16-hex>".
# The Serial_no must match this board's hostname suffix (imx8mn-ddr4-evk-<serial>).
```
...
```bash
# from uuu-target<N>/:
mfgtool-files-imx8mn-ddr4-evk/uuu mfgtool-files-imx8mn-ddr4-evk/full_image.uuu \
  2>&1 | tee uuu-flash-$(date -u +%Y%m%dT%H%M%SZ).log
# ~2 min; SDPS-boots mfgtool SPL → fastboot → flashes the four artifacts to eMMC.
...
Power off → restore `SW1101`/`SW1102` to the eMMC position (per the EVK silkscreen — see §0.1) → power on → watch `$SERIAL_LOG`. Expect:
...
```bash
fio@imx8mn-ddr4-evk:~$ ip -4 addr show eth0 | awk '/inet /{print $2}' | cut -d/ -f1
192.168.1.42
```
...
|---|---|---|
| `bootloader`     | SPL + DDR fw (`imx-boot-imx8mn-ddr4-evk`) | slot A — primary |
| `bootloader_s`   | identical copy | slot B — fallback for SPL |
| `bootloader2`    | fit-image (`u-boot-imx8mn-ddr4-evk.itb`) | slot A — primary fit-image |
| `bootloader2_s`  | identical copy | slot B — fallback for fit-image |

(The imx8mn does **not** use a `sit.bin` — there is no `sit` partition to inspect; the BootROM finds the bootloader from a fused offset. See §0.4.)
...
FIO: primary boot image offset = 0x0   /   primary FIT offset = 0x300
FIO: secondary boot image offset = 0x0   /   secondary FIT offset = 0x300    # identical to primary — see note
FIO: ###########################################
```

> **i.MX 8M Nano firmware-update path note.** The i.MX 8M Nano uses the *alternative*
> (backup/restore, no trial-boot) firmware-update path rather than the i.MX 8M Mini's regular
> A/B trial-boot scheme — there is **no distinct secondary firmware slot**. The debug block reports `secondary boot image offset = 0x0` /
> `secondary FIT offset = 0x300` — **identical to the primary offsets** (`0x0` / `0x300`).
> That primary==secondary equality is the alternative path's signature; the i.MX 8M Mini's
> regular A/B scheme instead shows *distinct* primary/secondary offsets.
...
| 1.1.2 | host | `fioctl devices show $DEVICE -f $FACTORY` | current target (expect `imx8mn-ddr4-evk-lmp-1`), tag (`main`), online state |
...
`fioctl targets tag` triggers a short CI metadata-update job; the new tag is live in the TUF metadata once that job finishes (~1-2 min). Verify with `fioctl targets show -f $FACTORY <N> | grep -A2 'Target: imx8mn-ddr4-evk-lmp-<N>'` — the `Tags:` line should list both `main` and `fio-test-plan`.
...
- `interface-test-imx8mn-evk.sh` available in this same directory — the executable interface test (23 §-sections, numbered §0–§22), already adapted to imx8mn hardware
...
`interface-test-imx8mn-evk.sh` is a self-contained device-side script — it runs all "always-on baseline" rows and skips destructive/optional ones (USB/SD with media, RTC persistence, memtester, WDT trigger, thermal-load).
...
**Step 1 — full results → `interface-test-results-imx8mn-<datetime>.md`.** Pipe the script to the device over SSH and capture its GitHub-flavored-markdown output (one table per §-section, plus the script's own PASS/FAIL/INFO/SKIP totals block):

```bash
TS=$(date -u +%Y%m%dT%H%M%SZ)
sshpass -p fio ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o LogLevel=ERROR \
  fio@$DEVICE_IP 'bash -s' < interface-test-imx8mn-evk.sh > interface-test-results-imx8mn-$TS.md
```

**Step 2 — high-level summary → `interface-test-summary-imx8mn-<datetime>.md`.** A one-row-per-§-section (per-interface) roll-up — the at-a-glance verdict view — derived from the Step-1 results. A section's verdict is FAIL if any of its rows FAILed, else PASS; the **Notes** column explains any FAIL by listing the failing check(s):

```bash
{
  printf '# Interface test — high-level summary by interface\n\n'
  printf '**Detail:** `interface-test-results-imx8mn-%s.md`\n\n' "$TS"
  ...
  awk -F'|' '
    ...
    END           { emit() }
  ' interface-test-results-imx8mn-$TS.md
} > interface-test-summary-imx8mn-$TS.md
```
...
| §10.x i.MX-specific | PASS | `fw_printenv \| grep ^fiovb\\.` returns the 9 expected vars; `cat /sys/devices/soc0/soc_id` matches `i.MX8MN`; CAAM RNG present at `/dev/hwrng` |
| §11.x USB (optional) | PASS for always-on baseline; rest skipped unless a device is plugged in | imx8mn USB is **ChipIdea (`ci_hdrc`), USB-2.0 only** — no xHCI / no USB-3.0. Always-on: ≥1 `ci_hdrc` controller bound under `/sys/bus/platform/drivers/ci_hdrc/`; UDC `ci_hdrc.0` in `/sys/class/udc/`. Root-hub count is **INFO** — `0` is fine if no port is in host mode. With a USB stick on a host-mode port: device enumerates and mounts. |
| §12.x microSD slot + eMMC health | PASS for always-on baseline; rest skipped unless a card is inserted | imx8mn has **3 mmc hosts**: `mmc0`=`30b40000.mmc` (SDIO Wi-Fi — the Broadcom BCM4345/6 combo, driver `brcmfmac`), `mmc1`=`30b50000.mmc` (the SD slot), `mmc2`=`30b60000.mmc` (eMMC, `mmcblk2`). Always-on (no card): `mmc1` present + bound to `30b50000.mmc`, no `/dev/mmcblk1*`. Card-detect-GPIO journal trace is INFO (not all revs log it). With a microSD inserted: `/dev/mmcblk1` appears, partitions mount. **eMMC health** (always-on baseline): detects the eMMC (the `mmcblk` whose sysfs `device/type`=MMC) → PASS; `pre_eol_info` → PASS at `0x01` Normal, FAIL at `0x02` Warning / `0x03` Urgent; `life_time` reported INFO — all read from sysfs, no `mmc-utils` in the image required. |
| §13.x RTC (optional) | PASS for always-on baseline; persistence + alarm checks skipped unless explicitly run | Always-on (no special hw): `/dev/rtc0` present, `/sys/class/rtc/rtc0/name = snvs_rtc 30370000.snvs:snvs-rtc-lp` (i.MX 8M Nano internal SNVS RTC), `hwclock --show` returns a parseable time matching system clock to within a second. **Expected baseline gotcha:** `/sys/class/rtc/rtc0/hctosys=0` and dmesg shows the boot RTC read returned epoch (1970-01-01) — this is normal on dev EVKs without the SNVS coin-cell battery installed; NTP brings the time back. Optional persistence test (cold-boot, see if RTC time survives) only PASSes if a battery is fitted. |
| §14.x memory | PASS for always-on baseline; stress test optional | Always-on (no extras): `MemTotal` ≥ 1,800,000 kB (the imx8mn-ddr4-evk has **2 GiB DDR4** — observed `1,929,656` kB after firmware reservations), `CmaTotal` = 655,360 kB (640 MiB device-tree CMA pool — verified on the imx8mn-ddr4-evk), `MemAvailable / MemTotal` > 0.5 at idle, `SwapTotal` non-zero (zram, ~1.8 GiB — see §1.x), no `out of memory` / `oom-kill` / `killed process` in the boot journal. Expected absences (don't false-FAIL): EDAC counters (non-ECC DDR4), `/proc/pressure/memory` (kernel may not have CONFIG_PSI). |
| §15.x QSPI/FlexSPI | NOT TESTED — expected-absence row | This build's devicetree leaves the i.MX 8M Nano FlexSPI controller (`30bb0000.spi`) disabled, so no NOR flash is visible to the kernel. Verdict is informational: the row documents what's *currently* off so a future build that enables FlexSPI causes a visible verdict flip (from "absent" to either PASS-with-MTD-checks or FAIL-controller-bound-but-no-chip). Until then, intentionally not exercised. |
| §16.x HW watchdog | PASS for always-on baseline; trigger test optional (destructive) | Always-on (no special hw): `/dev/watchdog0` present, `wdctl -O /dev/watchdog0` returns `IDENTITY="imx2+ watchdog" TIMEOUT="60"` (driver bound to `30280000.watchdog`, the i.MX 8M Nano internal WDT). systemd's `RebootWatchdogSec=60` means WDT is used as a *shutdown safety net*, but `RuntimeWatchdogUSec=0` — i.e. nothing kicks it during steady-state operation. Bootstatus flags all 0 (last reset was NOT WDT-induced). Optional trigger test resets the board on purpose. |
| §17.x temperature sensors | PASS for always-on baseline; load test optional | Always-on (no special hw): the i.MX 8M Nano TMU exposes **one** thermal zone — `thermal_zone0=cpu-thermal` (single sensor, no separate `soc-thermal`). Reads 15°C ≤ T ≤ 85°C at idle (observed ~52°C — the small bare EVK runs warm). Trip points are INFO — read from the device tree via sysfs (observed passive=85°C, critical=95°C). One cooling device: `cooling_device0=cpufreq-cpu0`. |
| §18.x Wayland / display | PASS on `lmp-wayland` builds; whole section N/A on plain `lmp` | **Only applies when DISTRO is `lmp-wayland`** (target 10 onward). Display path: the i.MX 8M Nano has no native HDMI — the imx8mn-evk has only MIPI-DSI (`32e10000.dsi_controller`, driver `imx_sec_dsim_drv`); HDMI needs the **optional external ADV7535 DSI→HDMI bridge board** on the DSI connector. When the ADV7535 is **not connected** (as on the target-10 bench) the kernel logs `failed to attach bridge … dsi_controller` / `adv7511 1-003d: Probe failed` and no `card0-HDMI-A-1` connector is created — **expected with the add-on absent, not a failure**; the `imx-drm` subsystem still binds the DSI controller. Always-on: `weston` service active + process running; `/dev/galcore` (Vivante 3D GPU at `38000000.gpu`, `galcore` 6.4.11) present; `/dev/dri/card0` + `/dev/dri/renderD128` present; `seatd` **optional/INFO** (logind libseat backend); a **3D GPU render check** — `weston-simple-egl` is run against the live compositor for ~5 s under `timeout`, PASS if it creates an EGL/GLES context and renders without crashing (exercises the GPU end-to-end — catches a UMD/kernel-driver mismatch or GPU hang that the node-presence checks miss; SKIP if `weston-simple-egl` is absent from the image). Display-attached checks are INFO — a `card0-HDMI-A-1` connector appears only when the ADV7535 bridge board is fitted. On a plain `lmp` build the entire row is N/A. |
| §19.x Wi-Fi radio | PASS for always-on baseline + scan | The imx8mn-ddr4-evk Wi-Fi is a **Broadcom BCM4345/6 (CYW43455)** combo on SDIO (`mmc0`), driver `brcmfmac`, station interface `wlan0` — not the i.MX 8M Mini's Qualcomm Atheros QCA9377/`ath10k_sdio`. Always-on: a `wl*` station interface present, the combo-chip driver module loaded, `wpa_supplicant` active, `nmcli radio` shows WIFI `enabled`. Scan check: bring the interface up → `nmcli device wifi rescan` → expect **≥1 AP** visible (proves the radio tunes and receives). `0` APs is a FAIL *unless* the bench is RF-isolated. Association → DHCP → traffic is the optional deeper test (needs AP credentials). |
| §20.x Bluetooth | PASS for always-on baseline + power-on/scan | The imx8mn-ddr4-evk Bluetooth is driven by **`btnxpuart`** (NXP UART BT), controller `hci0` on a UART bus — not the i.MX 8M Mini's QCA `btqca`. Note the boot log shows `hci0: Setting baudrate failed (-56)` / `Setting Power Save mode failed (-22)` — `hci0` still comes up and enumerates, but the link is not fully clean on the target-10 build. Always-on: `hci0` present (`hciconfig`), `bluetooth.service` active + `bluetoothd` running, `hci0` not rfkill-blocked, `bluetoothctl show` enumerates the controller. Power-on/scan check: `bluetoothctl power on` → `hci0 UP` → timed `scan on` → expect **≥1 device** discovered. `0` devices is a FAIL *unless* the bench is RF-isolated. Pairing/connect is the optional deeper test. |
| §21.x audio | PASS for the always-on baseline; tone playback skipped | ALSA is up with all on-board cards — `/proc/asound/cards` lists **3**: `imxspdif` (S/PDIF), `btscoaudio` (the BT-SCO `simple-card`), `wm8524audio` (the on-board WM8524 line/headphone DAC). `aplay -l` / `arecord -l` enumerate playback + capture devices, and `/dev/snd` has the `controlC*` / `pcmC*` nodes. Unlike the i.MX 8M Mini there is **no `micfilaudio`** (no PDM-mic card) on the imx8mn-ddr4-evk, and `sound-bt-sco` is *present* here (registered as `btscoaudio`), not deferred. `sound-ak4458` (optional audio expansion board) stays in deferred-probe — an **expected absence** on a bare EVK, reported INFO not FAIL. Actual tone playback needs a speaker/headset — skipped. |
| §22.x PCIe | N/A — documented absence | The i.MX 8M Nano has **no PCIe controller** — NXP dropped it on the Nano (the Mini, Plus and Quad all have PCIe). The script emits one INFO row confirming the absence: no `*.pcie` devicetree node and an empty `/sys/bus/pci/devices`. Never FAILs. |
...
The interface test exists as the executable **`interface-test-imx8mn-evk.sh`** (see §6.2) — 23
...
To extend or audit it, edit `interface-test-imx8mn-evk.sh` directly — each `header "§N …"` block
...
1. Power off → set `SW1101`/`SW1102` to USB-SDP (per the EVK silkscreen — §0.1) → power on → connect a USB-C cable to the `Download` port
2. Run uuu against the latest known-good `full_image.uuu` (re-fetch from `fioctl targets artifacts` if the local copy is stale)
3. Power off → restore `SW1101`/`SW1102` to eMMC (per the EVK silkscreen — §0.1) → power on
...
- Cortex-M7 co-processor firmware (loaded by U-Boot, separate update mechanism)
...
- [NXP i.MX 8M Nano EVK product page](https://www.nxp.com/part/8MNANOD4-EVK) — pinout, boot-mode switches, connector designators
- [uuu (Universal Update Utility) on GitHub](https://github.com/nxp-imx/mfgtools)
- Interface test: `interface-test-imx8mn-evk.sh` (the executable Phase 6 sweep — see §6.2)

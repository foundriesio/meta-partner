...
Factory: `nxp-imx` · Device: `imx8mm-evk-dev` · Target machine: `imx8mm-lpddr4-evk`

> ## i.MX 8M Mini EVK — board characteristics
>
> Key hardware facts this plan depends on:
>
> - **Display:** the i.MX 8M Mini has no HDMI controller — the imx8mm-evk has only a MIPI-DSI
>   interface (`32e10000.mipi_dsi`). HDMI output requires the **optional external ADV7535
>   DSI→HDMI bridge board** plugged into the DSI MIPI connector — it is not an on-board part.
>   With the ADV7535 fitted it presents a `card0-HDMI-A-1` DRM connector; with the bridge board
>   absent there is no HDMI output and no such connector.
> - **Companion core:** Cortex-M4.
> - **No NPU**; GPU is a Vivante GC NanoUltra + GC320.
> - **Memory:** the imx8mm-evk has 2 GiB LPDDR4 (~1,930,000 kB visible after firmware reservations).
> - **SDP USB ID:** `1fc9:0134` (HID-based SDP, two-stage `SDP:`+`SDPV:` flow).
> - **Wi-Fi/BT:** a Qualcomm Atheros QCA9377 combo — Wi-Fi via `ath10k_sdio` (SDIO on `mmc0`,
>   interface `wlan0`), Bluetooth via `btqca` over `hci_uart`.
> - **uuu flash needs five artifacts** including a `sit.bin` (Secondary Image Table).
...
## Boot chain (imx8mm-evk, FoundriesFactory LmP)
...
| 2. **SPL** + DDR firmware | `imx-boot-<machine>` (e.g. `imx-boot-imx8mm-lpddr4-evk`); flashed by uuu into eMMC partitions `bootloader` (slot A) and `bootloader_s` (slot B) | BootROM (AHAB signature) | SRK fuses |
| 3. **fit-image** — ATF (BL31) + OP-TEE (BL32) + U-Boot proper (BL33) + boot script | published as a *separate* artifact `u-boot-<machine>.itb` (e.g. `u-boot-imx8mm-lpddr4-evk.itb`); flashed by uuu into eMMC partitions `bootloader2` (slot A) and `bootloader2_s` (slot B). **A `sit.bin`** (`sit-imx8mm-lpddr4-evk.bin`, a 512-byte Secondary Image Table) is also flashed to the `sit` partition — it tells the BootROM where the bootloader images live. | LmP-patched SPL via FIT signature check | **Factory-specific public key embedded in SPL's devicetree at build time** |
...
**When Claude executes this plan, it should call `AskUserQuestion` at the start to collect all variables below** rather than hardcoding values. Suggested defaults reflect a typical lab bench but the plan should work for any Factory whose i.MX 8M Mini EVK target uses the `imx8mm-lpddr4-evk` MACHINE.

| Variable | Suggested default | Notes |
|---|---|---|
| `$FACTORY` | `nxp-imx` | the Factory the device registers to |
| `$DEVICE` | `imx8mm-evk-dev` | a fresh device name; bump on re-runs (registration is one-shot) |
...
| `$SERIAL_DEV` | `/dev/ttyUSB1` | A53 debug UART — **confirmed on this bench**. The imx8mm-evk's debug bridge is a dual-UART (2 ttyUSB devices): `ttyUSB0` is silent (M4/secondary), `ttyUSB1` carries the A53 Linux console. |
...
- imx8mm-evk hardware on the bench, powered via USB-C PD (the 8MMini-BB base board has no barrel jack), Ethernet plugged into the lab LAN, debug microUSB cable (to the `DEBUG` connector — see §0.2) to the host
...
## Phase 0 — Lab setup (imx8mm-evk)

### 0.1 Boot-mode switch cheat sheet

The imx8mm-evk selects its boot device with two boot-mode DIP switches, **`SW1101`** and
...
that silkscreen (and cross-check the i.MX 8M Mini EVK hardware user guide if the silkscreen is
...
| Power | **USB-C**, labelled **`POWER`** (USB-C PD — the 8MMini-BB has no barrel jack) | 5 V / 3 A | required even with the `Download` port attached |
...
# Five separate artifacts — all required (the imx8mm flash includes a sit.bin):
fioctl targets artifacts -f $FACTORY $TARGET \
  imx8mm-lpddr4-evk/lmp-factory-image-imx8mm-lpddr4-evk.wic.gz > lmp-factory-image-imx8mm-lpddr4-evk.wic.gz
fioctl targets artifacts -f $FACTORY $TARGET \
  imx8mm-lpddr4-evk/imx-boot-imx8mm-lpddr4-evk > imx-boot-imx8mm-lpddr4-evk
fioctl targets artifacts -f $FACTORY $TARGET \
  imx8mm-lpddr4-evk/u-boot-imx8mm-lpddr4-evk.itb > u-boot-imx8mm-lpddr4-evk.itb
fioctl targets artifacts -f $FACTORY $TARGET \
  imx8mm-lpddr4-evk/sit-imx8mm-lpddr4-evk.bin > sit-imx8mm-lpddr4-evk.bin
fioctl targets artifacts -f $FACTORY $TARGET \
  imx8mm-lpddr4-evk-mfgtools/mfgtool-files-imx8mm-lpddr4-evk.tar.gz > mfgtool-files.tar.gz

tar xzf mfgtool-files.tar.gz              # creates mfgtool-files-imx8mm-lpddr4-evk/ with uuu, full_image.uuu, bundled mfgtool SPL/fit
# do NOT gunzip the wic — uuu's `flash -raw2sparse` reads .wic.gz directly
```

Two non-obvious things to know about these artifacts:

- **The wic is `lmp-factory-image-…`** — FoundriesFactory CI on i.MX targets publishes the factory-image variant (which includes Foundries-specific extras), not `lmp-base-console-image-…`. Adjust the artifact name if your Factory's CI publishes a different variant; check via `fioctl targets artifacts -f $FACTORY $TARGET | grep '^imx8mm-lpddr4-evk/' | grep wic`.
- **The `mfgtool-files` tarball is published under a *separate* build run** named `<machine>-mfgtools/`, *not* under the main machine path. So the mfg artifact path is `imx8mm-lpddr4-evk-mfgtools/mfgtool-files-imx8mm-lpddr4-evk.tar.gz` — note the `-mfgtools` suffix on the run name. uuu, `full_image.uuu`, the bootstrap mfgtool SPL (`imx-boot-mfgtool`), and a bootstrap fit-image (`u-boot-mfgtool.itb`) all live inside this tarball.
...
FB: flash -raw2sparse all ../lmp-factory-image-imx8mm-lpddr4-evk.wic.gz/*
FB: flash bootloader     ../imx-boot-imx8mm-lpddr4-evk      # SPL → eMMC "bootloader" partition (slot A)
FB: flash bootloader2    ../u-boot-imx8mm-lpddr4-evk.itb    # fit-image → "bootloader2" partition (slot A)
FB: flash bootloader_s   ../imx-boot-imx8mm-lpddr4-evk      # SPL → "bootloader_s" (slot B, redundant)
FB: flash bootloader2_s  ../u-boot-imx8mm-lpddr4-evk.itb    # fit-image → "bootloader2_s" (slot B)
FB: flash sit            ../sit-imx8mm-lpddr4-evk.bin       # Secondary Image Table → "sit" partition
FB: ucmd mmc partconf ${emmc_dev} ${emmc_ack} 1 0
FB: done
```

Maps directly to the boot architecture: SPL goes to `bootloader{,_s}`, fit-image goes to `bootloader2{,_s}`, two slots each so fiovb has somewhere to fall back to. The flash also writes a **`sit.bin`** — `sit-imx8mm-lpddr4-evk.bin`, a 512-byte Secondary Image Table that tells the BootROM where the bootloader images live. It is published as the *separate artifact* `imx8mm-lpddr4-evk/sit-imx8mm-lpddr4-evk.bin` and must be pulled alongside the others (so **five** artifacts total). The SDP flow uses `SDP:` + `SDPV:` — the two-stage HID Serial Download protocol.
...
# Expected: "1fc9:0134 …" — the i.MX 8M Mini SDP USB ID (HID-based Serial Download protocol).
# (Matches uuu's built-in device table entry: "SDP: MX8MM 0x1fc9 0x0134".)
```
...
```bash
# from uuu-target<N>/:
mfgtool-files-imx8mm-lpddr4-evk/uuu mfgtool-files-imx8mm-lpddr4-evk/full_image.uuu \
  2>&1 | tee uuu-flash-$(date -u +%Y%m%dT%H%M%SZ).log
# ~2 min; SDP-boots mfgtool SPL → fastboot → flashes the five artifacts to eMMC.
...
```bash
fio@imx8mm-lpddr4-evk:~$ ip -4 addr show eth0 | awk '/inet /{print $2}' | cut -d/ -f1
192.168.1.42
```
...
|---|---|---|
| `bootloader`     | SPL + DDR fw (`imx-boot-imx8mm-lpddr4-evk`) | slot A — primary |
| `bootloader_s`   | identical copy | slot B — fallback for SPL |
| `bootloader2`    | fit-image (`u-boot-imx8mm-lpddr4-evk.itb`) | slot A — primary fit-image |
| `bootloader2_s`  | identical copy | slot B — fallback for fit-image |

(The imx8mm **does** use a `sit.bin` — `sit-imx8mm-lpddr4-evk.bin` flashed to the `sit` partition — see §0.4. This is one of the artifacts the §0.4 uuu flow pulls.)
...
| 1.1.2 | host | `fioctl devices show $DEVICE -f $FACTORY` | current target (expect `imx8mm-lpddr4-evk-lmp-1`), tag (`main`), online state |
...
`fioctl targets tag` triggers a short CI metadata-update job; the new tag is live in the TUF metadata once that job finishes (~1-2 min). Verify with `fioctl targets show -f $FACTORY <N> | grep -A2 'Target: imx8mm-lpddr4-evk-lmp-<N>'` — the `Tags:` line should list both `main` and `fio-test-plan`.
...
- `interface-test-imx8mm-evk.sh` available in this same directory — the executable interface test (23 §-sections, numbered §0–§22), already adapted to imx8mm hardware
...
`interface-test-imx8mm-evk.sh` is a self-contained device-side script — it runs all "always-on baseline" rows and skips destructive/optional ones (USB/SD with media, RTC persistence, memtester, WDT trigger, thermal-load).
...
**Step 1 — full results → `interface-test-results-imx8mm-<datetime>.md`.** Pipe the script to the device over SSH and capture its GitHub-flavored-markdown output (one table per §-section, plus the script's own PASS/FAIL/INFO/SKIP totals block):

```bash
TS=$(date -u +%Y%m%dT%H%M%SZ)
sshpass -p fio ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o LogLevel=ERROR \
  fio@$DEVICE_IP 'bash -s' < interface-test-imx8mm-evk.sh > interface-test-results-imx8mm-$TS.md
```

**Step 2 — high-level summary → `interface-test-summary-imx8mm-<datetime>.md`.** A one-row-per-§-section (per-interface) roll-up — the at-a-glance verdict view — derived from the Step-1 results. A section's verdict is FAIL if any of its rows FAILed, else PASS; the **Notes** column explains any FAIL by listing the failing check(s):

```bash
{
  printf '# Interface test — high-level summary by interface\n\n'
  printf '**Detail:** `interface-test-results-imx8mm-%s.md`\n\n' "$TS"
  ...
  awk -F'|' '
    ...
    END           { emit() }
  ' interface-test-results-imx8mm-$TS.md
} > interface-test-summary-imx8mm-$TS.md
```
...
| §10.x i.MX-specific | PASS | `fw_printenv \| grep ^fiovb\\.` returns the 9 expected vars; `cat /sys/devices/soc0/soc_id` matches `i.MX8MM`; CAAM RNG present at `/dev/hwrng`; a **conditional G2D 2D-engine check** exercises the dedicated 2D core (GC320) by running a g2d sample tool when `libg2d` + the tool are present in the image — SKIP otherwise (the common case) |
| §11.x USB (optional) | PASS for always-on baseline; rest skipped unless a device is plugged in | imx8mm USB is **ChipIdea (`ci_hdrc`), USB-2.0 only** — no xHCI / no USB-3.0. Always-on: ≥1 `ci_hdrc` controller bound under `/sys/bus/platform/drivers/ci_hdrc/`; UDC `ci_hdrc.0` in `/sys/class/udc/`. Root-hub count is **INFO** — `0` is fine if no port is in host mode. With a USB stick on a host-mode port: device enumerates and mounts. |
| §12.x microSD slot + eMMC health | PASS for always-on baseline; rest skipped unless a card is inserted | imx8mm has **3 mmc hosts**: `mmc0`=`30b40000.mmc` (SDIO Wi-Fi, QCA9377), `mmc1`=`30b50000.mmc` (the SD slot), `mmc2`=`30b60000.mmc` (eMMC, `mmcblk2`). Always-on (no card): `mmc1` present + bound to `30b50000.mmc`, no `/dev/mmcblk1*`. Card-detect-GPIO journal trace is INFO (not all revs log it). With a microSD inserted: `/dev/mmcblk1` appears, partitions mount. **eMMC health** (always-on baseline): detects the eMMC (the `mmcblk` whose sysfs `device/type`=MMC) → PASS; `pre_eol_info` → PASS at `0x01` Normal, FAIL at `0x02` Warning / `0x03` Urgent; `life_time` reported INFO — all read from sysfs, no `mmc-utils` in the image required. |
| §13.x RTC (optional) | PASS for always-on baseline; persistence + alarm checks skipped unless explicitly run | Always-on (no special hw): `/dev/rtc0` present, `/sys/class/rtc/rtc0/name = snvs_rtc 30370000.snvs:snvs-rtc-lp` (i.MX 8M Mini internal SNVS RTC), `hwclock --show` returns a parseable time matching system clock to within a second. **Expected baseline gotcha:** `/sys/class/rtc/rtc0/hctosys=0` and dmesg shows the boot RTC read returned epoch (1970-01-01) — this is normal on dev EVKs without the SNVS coin-cell battery installed; NTP brings the time back. Optional persistence test (cold-boot, see if RTC time survives) only PASSes if a battery is fitted. |
| §14.x memory | PASS for always-on baseline; stress test optional | Always-on (no extras): `MemTotal` ≥ 1,800,000 kB (the imx8mm-evk's **2 GiB** LPDDR4 minus firmware reservations — observed ~1,930,000 kB), `CmaTotal` = 655,360 kB (640 MiB device-tree CMA pool), `MemAvailable / MemTotal` > 0.5 at idle, `SwapTotal` non-zero (zram, ~1.8 GiB — see §1.x), no `out of memory` / `oom-kill` / `killed process` in the boot journal. Expected absences (don't false-FAIL): EDAC counters (non-ECC LPDDR4), `/proc/pressure/memory` (kernel may not have CONFIG_PSI). |
| §15.x QSPI/FlexSPI | NOT TESTED — expected-absence row | This build's devicetree leaves the i.MX 8M Mini FlexSPI controller (`30bb0000.spi`) disabled, so no NOR flash is visible to the kernel. Verdict is informational: the row documents what's *currently* off so a future build that enables FlexSPI causes a visible verdict flip (from "absent" to either PASS-with-MTD-checks or FAIL-controller-bound-but-no-chip). Until then, intentionally not exercised. |
| §16.x HW watchdog | PASS for always-on baseline; trigger test optional (destructive) | Always-on (no special hw): `/dev/watchdog0` present, `wdctl -O /dev/watchdog0` returns `IDENTITY="imx2+ watchdog" TIMEOUT="60"` (driver bound to `30280000.watchdog`, the i.MX 8M Mini internal WDT). systemd's `RebootWatchdogSec=60` means WDT is used as a *shutdown safety net*, but `RuntimeWatchdogUSec=0` — i.e. nothing kicks it during steady-state operation. Bootstatus flags all 0 (last reset was NOT WDT-induced). Optional trigger test resets the board on purpose. |
| §17.x temperature sensors | PASS for always-on baseline; load test optional | Always-on (no special hw): the i.MX 8M Mini TMU exposes **one** thermal zone — `thermal_zone0=cpu-thermal` (single sensor, no separate `soc-thermal`). Reads 15°C ≤ T ≤ 85°C at idle (observed ~52°C — the small bare EVK runs warm). Trip points are INFO — ⚠️ verify against the device tree. One cooling device: `cooling_device0=cpufreq-cpu0`. |
| §18.x Wayland / display | PASS on `lmp-wayland` builds; whole section N/A on plain `lmp` | **Only applies when DISTRO is `lmp-wayland`** (target 10 onward). Display path: the i.MX 8M Mini has no native HDMI — the imx8mm-evk has only MIPI-DSI (`32e10000.mipi_dsi`); HDMI needs the **optional external ADV7535 DSI→HDMI bridge board** on the DSI connector. With the ADV7535 fitted it presents a `card0-HDMI-A-1` DRM connector; with the bridge board absent no such connector appears (the `adv7535@3d` node fails to probe) — expected, not a failure. Always-on: `weston` service active + process running; `/dev/galcore` (Vivante GPU) present; `/dev/dri/card0` + `/dev/dri/renderD128` present; `seatd` **optional/INFO** (logind libseat backend); a **3D GPU render check** — `weston-simple-egl` is run against the live compositor for ~5 s under `timeout`, PASS if it creates an EGL/GLES context and renders without crashing (exercises the GPU end-to-end — catches a UMD/kernel-driver mismatch or GPU hang that the node-presence checks miss; SKIP if `weston-simple-egl` is absent from the image). Display-attached checks (`card0-HDMI-A-1` status, mode, `fb0` size) are INFO unless a monitor is known to be plugged in. On a plain `lmp` build the entire row is N/A. |
| §19.x Wi-Fi radio | PASS for always-on baseline + scan | The imx8mm-evk Wi-Fi is a **Qualcomm Atheros QCA9377** (SDIO on `mmc0`). Always-on: station interface `wlan0` present, driver `ath10k_sdio` (+ `ath10k_core`, `ath`) loaded, `wpa_supplicant` active, `nmcli radio` shows WIFI `enabled`. Scan check: bring `wlan0` up → `nmcli device wifi rescan` → expect **≥1 AP** visible (proves the radio tunes and receives). `0` APs is a FAIL *unless* the bench is RF-isolated. Association → DHCP → traffic is the optional deeper test (needs AP credentials). |
| §20.x Bluetooth | PASS for always-on baseline + power-on/scan | The imx8mm-evk Bluetooth is the QCA combo chip — driver `btqca` over `hci_uart`. Always-on: `hci0` present (`hciconfig`), `bluetooth.service` active + `bluetoothd` running, `hci0` not rfkill-blocked, `bluetoothctl show` enumerates the controller. Power-on/scan check: `bluetoothctl power on` → `hci0 UP` → timed `scan on` → expect **≥1 device** discovered. `0` devices is a FAIL *unless* the bench is RF-isolated. Pairing/connect is the optional deeper test. |
| §21.x audio | PASS for the always-on baseline; tone playback skipped | ALSA is up with all on-board cards — `/proc/asound/cards` lists **3**: `imxspdif` (S/PDIF), `wm8524audio` (the on-board WM8524 line/headphone DAC), `micfilaudio` (PDM mic). `aplay -l` / `arecord -l` enumerate playback + capture devices, and `/dev/snd` has the `controlC*` / `pcmC*` nodes. `sound-ak4458` (optional audio expansion board) and `sound-bt-sco` stay in deferred-probe — **expected absences** on a bare EVK, reported INFO not FAIL. Actual tone playback needs a speaker/headset — skipped. |
| §22.x PCIe | PASS only with a fitted card; otherwise INFO — never FAIL | The i.MX 8M Mini has one PCIe Gen2 controller (DesignWare-based, driver `imx6q-pcie`). Always-on (no card): the script reports the `*.pcie` devicetree node, whether the host bridge bound, and the `/sys/bus/pci/devices` count — all **INFO**, since a PCIe link only comes up with an endpoint card fitted in the slot (a bare bench legitimately shows no link / "PHY link never came up"). It goes **PASS** only when an endpoint enumerates past the host bridge; the link+traffic test is **SKIP** (needs a card). |
...
The interface test exists as the executable **`interface-test-imx8mm-evk.sh`** (see §6.2) — 23
...
To extend or audit it, edit `interface-test-imx8mm-evk.sh` directly — each `header "§N …"` block
...
- [NXP i.MX 8M Mini EVK product page](https://www.nxp.com/products/8MMINIEVK) — pinout, boot-mode switches, connector designators (⚠️ confirm the exact URL/board user-guide for your EVK revision)
- [uuu (Universal Update Utility) on GitHub](https://github.com/nxp-imx/mfgtools)
- Interface test: `interface-test-imx8mm-evk.sh` (the executable Phase 6 sweep — see §6.2)

#!/bin/bash
# ============================================================================
# Interface test for the imx8mn-evk — device-side script.
# Runs all "always-on baseline" rows from fio-test-plan-imx8mn-evk.md
...
#       -o LogLevel=ERROR fio@<DEVICE_IP> 'bash -s' < interface-test-imx8mn-evk.sh \
...
[ "$si" = "i.MX8MN" ] && { row "\`soc_id\`" "\`$si\`" "PASS"; P; } || { row "\`soc_id\`" "\`$si\`" "FAIL"; F; }
hb=$(SUDO head -c8 /dev/hwrng 2>/dev/null | xxd -p)
[ -n "$hb" ] && { row "CAAM RNG (\`/dev/hwrng\`)" "8 bytes: \`$hb\`" "PASS"; P; } || { row "CAAM RNG" "no read" "FAIL"; F; }
# The i.MX 8M Nano has NO VPU (hardware video codec) — NXP dropped it on the Nano (the Mini
# has one). Absent VPU nodes are expected here, so this is a documented-absence INFO row.
vp=""; for d in /dev/mxc_hantro /dev/mxc_hantro_h1 /dev/video0; do [ -e "$d" ] && vp="$vp $d"; done
[ -n "$vp" ] && { row "VPU device nodes (i.MX 8M Nano has no VPU — unexpected)" "\`$vp\`" "INFO"; I; } || { row "VPU device nodes" "absent — expected (i.MX 8M Nano has no VPU)" "INFO"; I; }
...
# imx8mn USB is ChipIdea (ci_hdrc), USB-2.0 only — NOT xHCI/USB-3.0 like the imx8mp
...
# imx8mn has 3 mmc hosts: mmc0=30b40000.mmc (SDIO Wi-Fi — Broadcom BCM4345/6, brcmfmac),
# mmc1=30b50000.mmc (SD slot), mmc2=30b60000.mmc (eMMC)
...
# imx8mn-ddr4-evk: 2 GiB DDR4 — observed MemTotal ~1,929,656 kB after firmware reservations
[ "$mt" -ge 1800000 ] && { row "\`MemTotal\`" "\`$mt\` kB (≥1,800,000 — 2 GiB DDR4)" "PASS"; P; } || { row "\`MemTotal\`" "\`$mt\` kB (expected ~1,929,656 — 2 GiB DDR4)" "FAIL"; F; }
cma=$(awk '/^CmaTotal:/{print $2}' /proc/meminfo)
# imx8mn-ddr4-evk CMA pool: 655360 kB (640 MiB) — verified on the imx8mn-ddr4-evk device tree
[ "$cma" = "655360" ] && { row "\`CmaTotal\`" "\`$cma\` kB (640 MiB)" "PASS"; P; } || { row "\`CmaTotal\`" "\`$cma\` kB (expected 655360 — 640 MiB)" "INFO"; I; }
...
# imx8mn TMU exposes ONE zone (cpu-thermal) — the imx8mp had two (cpu + soc)
zc=$(ls /sys/class/thermal/ | grep -c '^thermal_zone')
[ "$zc" -eq 1 ] && { row "Thermal zones" "\`$zc\` (imx8mn TMU = single \`cpu-thermal\` sensor)" "PASS"; P; } || { row "Thermal zones" "\`$zc\` (expected 1)" "FAIL"; F; }
...
row "\`thermal_zone0\` trip points" "passive=\`${pa}°C\` critical=\`${cr}°C\` (from the device tree)" "INFO"; I
...
# imx8mn has no native HDMI — display is MIPI-DSI (32e10000.dsi_controller, imx_sec_dsim_drv).
# HDMI requires the OPTIONAL external ADV7535 DSI->HDMI bridge board on the DSI connector. If it
# is not connected, the adv7535@3d node fails to probe and no card0-HDMI-A-1 connector appears
# — expected when the add-on is absent, not a failure; the connector check below stays INFO.
# GPU is a single Vivante 3D core at 38000000.gpu (galcore 6.4.11), exposed at /dev/galcore.
...
if [ -n "$wlif" ]; then
  ...
  # imx8mn-ddr4-evk Wi-Fi: Broadcom BCM4345/6 (CYW43455) on SDIO, driver brcmfmac, iface wlan0
  # (not the imx8mm's Qualcomm Atheros QCA9377/ath10k).
  drv=$(lsmod 2>/dev/null | awk '/^(brcmfmac|brcmutil|ath10k|ath11k|ath6kl|ath|moal|mlan|nxp)/{print $1}' | paste -sd, -)
  [ -n "$drv" ] && { row "Wi-Fi driver module (expect \`brcmfmac\` — BCM4345/6)" "\`$drv\`" "PASS"; P; } || { row "Wi-Fi driver module" "none loaded (expected \`brcmfmac\`)" "FAIL"; F; }
...
if echo fio | sudo -S -p '' hciconfig hci0 >/dev/null 2>&1; then
  ...
  # imx8mn-ddr4-evk Bluetooth: driver btnxpuart (NXP UART BT), hci0 on a UART bus (not the
  # imx8mm's QCA btqca). Boot log notes baudrate/power-save warnings — hci0 still comes up.
  bdrv=$(lsmod 2>/dev/null | awk '/^(btqca|hci_uart|btnxpuart|btrtl|btbcm)/{print $1}' | paste -sd, -)
  [ -n "$bdrv" ] && { row "BT driver module (expect \`btnxpuart\`)" "\`$bdrv\`" "PASS"; P; } || { row "BT driver module" "none loaded (expected \`btnxpuart\`)" "FAIL"; F; }
...
header "§21 Audio"
# imx8mn-evk ALSA — working cards: imxspdif (S/PDIF), btscoaudio (BT-SCO simple-card),
# wm8524audio (on-board WM8524 line/headphone DAC). No micfilaudio (no PDM-mic card) on
# this board. sound-ak4458 (optional audio expansion board) stays in deferred-probe.
acards=$(grep -cE '^ *[0-9]+ \[' /proc/asound/cards 2>/dev/null)
[ "${acards:-0}" -ge 3 ] && { row "ALSA sound cards (\`/proc/asound/cards\`)" "\`$acards\` (≥3: imxspdif, btsco, wm8524)" "PASS"; P; } || { row "ALSA sound cards" "\`${acards:-0}\` (expected ≥3)" "FAIL"; F; }
...
row "Deferred-probe audio nodes" "sound-ak4458 stays unbound — expected (optional audio expansion board absent)" "INFO"; I
row "Tone-playback test" "skipped (optional — needs a speaker/headset)" "SKIP"; S

header "§22 PCIe"
# The i.MX 8M Nano has NO PCIe controller — NXP dropped it on the Nano (the Mini, Plus and Quad
# all have PCIe). Absent PCIe is expected here, so this is a documented-absence INFO row.
...
if [ -z "$pcienode" ] && [ "${pcin:-0}" -eq 0 ]; then
  row "PCIe controller" "absent — expected (i.MX 8M Nano has no PCIe)" "INFO"; I
else
  row "PCIe controller (i.MX 8M Nano has no PCIe — unexpected)" "node=\`${pcienode:-none}\` pci_devices=\`$pcin\`" "INFO"; I
fi

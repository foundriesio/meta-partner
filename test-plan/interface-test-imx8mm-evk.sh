#!/bin/bash
# ============================================================================
# Interface test for the imx8mm-evk — device-side script.
# Runs all "always-on baseline" rows from fio-test-plan-imx8mm-evk.md
...
#       -o LogLevel=ERROR fio@<DEVICE_IP> 'bash -s' < interface-test-imx8mm-evk.sh \
...
cat <<MD
# Interface test results — imx8mm-evk on \`nxp-imx\`
...
[ "$si" = "i.MX8MM" ] && { row "\`soc_id\`" "\`$si\`" "PASS"; P; } || { row "\`soc_id\`" "\`$si\`" "FAIL"; F; }
...
# G2D 2D-engine check — the i.MX 8M Mini has a dedicated Vivante 2D core (GC320). Exercise it via
...
if [ -z "$g2dlib" ]; then
  row "G2D 2D engine (GC320)" "no \`libg2d\` in image — G2D not enabled in this build; not run" "SKIP"; S
elif [ ! -e /dev/galcore ]; then
  row "G2D 2D engine (GC320)" "\`/dev/galcore\` absent — GPU stack not up; not run" "SKIP"; S
elif [ -z "$g2dbin" ]; then
  row "G2D 2D engine (GC320)" "\`libg2d\` present but no g2d sample tool to exercise it; not run" "SKIP"; S
else
  g2dout=$(SUDO timeout -k 2 25 "$g2dbin"); rc=$?
  if [ "$rc" = 0 ]; then
    row "G2D 2D blit (\`$g2dbin\`)" "ran clean — dedicated 2D core (GC320) exercised via the G2D API" "PASS"; P
  else
...
# imx8mm USB is ChipIdea (ci_hdrc), USB-2.0 only — NOT xHCI/USB-3.0 like the imx8mp
cic=$(ls /sys/bus/platform/drivers/ci_hdrc/ 2>/dev/null | grep -vE '^(bind|unbind|uevent|module)$' | wc -l)
[ "$cic" -ge 1 ] && { row "ChipIdea USB controllers bound (\`ci_hdrc\`)" "\`$cic\`" "PASS"; P; } || { row "ChipIdea USB controllers" "none bound" "FAIL"; F; }
...
# imx8mm has 3 mmc hosts: mmc0=30b40000.mmc (SDIO Wi-Fi, QCA9377), mmc1=30b50000.mmc (SD slot), mmc2=30b60000.mmc (eMMC)
...
[ "$md" = "30b50000.mmc" ] && { row "\`mmc1\` bound to \`30b50000.mmc\` (SD slot)" "yes" "PASS"; P; } || { row "\`mmc1\` device" "\`$md\`" "FAIL"; F; }
...
# imx8mm-evk has 2 GiB LPDDR4 → ~1,930,000 kB visible after firmware reservations
[ "$mt" -ge 1800000 ] && { row "\`MemTotal\`" "\`$mt\` kB (≥1,800,000 — 2 GiB board)" "PASS"; P; } || { row "\`MemTotal\`" "\`$mt\` kB" "FAIL"; F; }
cma=$(awk '/^CmaTotal:/{print $2}' /proc/meminfo)
[ "$cma" = "655360" ] && { row "\`CmaTotal\`" "\`$cma\` kB (640 MiB)" "PASS"; P; } || { row "\`CmaTotal\`" "\`$cma\` kB (expected 655360)" "FAIL"; F; }
...
# imx8mm TMU exposes ONE zone (cpu-thermal) — the imx8mp had two (cpu + soc)
zc=$(ls /sys/class/thermal/ | grep -c '^thermal_zone')
[ "$zc" -eq 1 ] && { row "Thermal zones" "\`$zc\` (imx8mm TMU = single \`cpu-thermal\` sensor)" "PASS"; P; } || { row "Thermal zones" "\`$zc\` (expected 1)" "FAIL"; F; }
...
row "\`thermal_zone0\` trip points" "passive=\`${pa}°C\` critical=\`${cr}°C\` (⚠️ verify against device tree)" "INFO"; I
...
# imx8mm has no native HDMI — display is MIPI-DSI (32e10000.mipi_dsi). HDMI requires the
# OPTIONAL external ADV7535 DSI->HDMI bridge board on the DSI connector. If it is not connected,
# the adv7535@3d node fails to probe and no card0-HDMI-A-1 connector appears — expected when the
# add-on is absent, not a failure; the connector check below stays INFO when the bridge is absent.
...
if systemctl list-unit-files weston.service >/dev/null 2>&1 && [ -e /dev/galcore ]; then
  ...
  if [ "$hs" = "connected" ]; then
    ...
    row "HDMI connector" "\`${hs:-absent}\` (INFO — a connector appears only with the optional ADV7535 bridge board fitted)" "INFO"; I
  fi
...
if [ -n "$wlif" ]; then
  ...
  # imx8mm-evk uses a Qualcomm Atheros QCA9377 (SDIO on mmc0) — ath10k, not the imx8mp's NXP moal/mlan
  drv=$(lsmod 2>/dev/null | awk '/^(ath10k|ath11k|ath6kl|ath)/{print $1}' | paste -sd, -)
  [ -n "$drv" ] && { row "Wi-Fi driver module (QCA \`ath*\`)" "\`$drv\`" "PASS"; P; } || { row "Wi-Fi driver module" "none loaded" "FAIL"; F; }
...
if echo fio | sudo -S -p '' hciconfig hci0 >/dev/null 2>&1; then
  ...
  # imx8mm-evk BT is the QCA combo chip — btqca over hci_uart (not the imx8mp's btnxpuart)
  bdrv=$(lsmod 2>/dev/null | awk '/^(btqca|hci_uart|btnxpuart|btrtl|btbcm)/{print $1}' | paste -sd, -)
...
header "§21 Audio"
# imx8mm-evk ALSA — working cards: imxspdif (S/PDIF), wm8524audio (on-board WM8524
# line/headphone DAC), micfilaudio (PDM mic). sound-ak4458 (optional audio expansion
# board) and sound-bt-sco stay in deferred-probe — expected absences on a bare EVK.
acards=$(grep -cE '^ *[0-9]+ \[' /proc/asound/cards 2>/dev/null)
[ "${acards:-0}" -ge 3 ] && { row "ALSA sound cards (\`/proc/asound/cards\`)" "\`$acards\` (≥3: imxspdif, wm8524, micfil)" "PASS"; P; } || { row "ALSA sound cards" "\`${acards:-0}\` (expected ≥3)" "FAIL"; F; }
grep -q wm8524 /proc/asound/cards 2>/dev/null && { row "On-board codec card (\`wm8524\` DAC)" "registered" "PASS"; P; } || { row "On-board codec (\`wm8524\`)" "not registered" "FAIL"; F; }
...
row "Deferred-probe audio nodes" "sound-ak4458 / sound-bt-sco stay unbound — expected (optional audio board / BT-SCO codec absent)" "INFO"; I
row "Tone-playback test" "skipped (optional — needs a speaker/headset)" "SKIP"; S

header "§22 PCIe"
# imx8mm has one PCIe Gen2 controller (DesignWare-based, driver imx6q-pcie). A PCIe link comes

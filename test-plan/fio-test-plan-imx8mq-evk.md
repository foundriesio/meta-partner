...
Factory: `nxp-imx` · Device: `imx8mq-evk-dev` · Target machine: `imx8mq-evk`

> ## i.MX 8M Quad EVK — board characteristics
>
> Key hardware facts this plan depends on:
>
> - **USB:** the i.MX 8M Quad has **DWC3 USB-3.0 (xHCI)** controllers (`xhci-hcd` / `dwc3`) —
>   *not* the i.MX 8M Mini's ChipIdea (`ci_hdrc`) USB-2.0-only controllers. Root hubs present
>   both a USB-2.0 (`1d6b:0002`) and a USB-3.0 (`1d6b:0003`) hub. The two DWC3 controllers are
>   `usb@38100000` / `usb@38200000` (`fsl,imx8mq-dwc3` + `snps,dwc3`), bound as `38100000.usb` /
>   `38200000.usb`.
> - **Display:** the i.MX 8M Quad EVK drives display through the **DCSS (Display Controller
>   Subsystem)** plus a Cadence HDP / `cdns-mhdp` HDMI controller, and also has a MIPI-DSI path.
>   The DRM connector is `card0-HDMI-A-1`.
> - **Companion core:** Cortex-M4.
> - **No NPU.** GPU is a **single Vivante GC7000Lite** — galcore
>   (`/sys/kernel/debug/gc/info`) reports `model 0x7000`, `revision 0x6214`,
>   `product 70002`; node `gpu3d@38000000` (`fsl,imx8mq-gpu`). The i.MX8MQ has **no separate
>   2D/VG GPU core** (unlike the 8M Mini's GC320 or 8M Plus's GC520L), so galcore's
>   `clk_get 2d core clock failed, disable 2d/vg!` at probe is **expected, not a fault**. 2D is
>   not absent though: the GC7000Lite is a 2D/3D core and NXP's G2D API runs 2D blits on it
>   (this LmP image ships only the GLES 3D stack — `libGAL`/`libGLESv2`/`libVSC`, no `libg2d`).
> - **Memory:** the imx8mq-evk has **3 GiB LPDDR4** — `MemTotal` reads **2,957,640 kB**
>   after firmware/CMA reservations; `CmaTotal` = 917,504 kB.
> - **SDP USB ID:** `1fc9:0152` — HID-based SDP, two-stage `SDP:`+`SDPV:` flow.
> - **Wi-Fi/BT:** on the M.2 slot (swappable, so chip/driver are module-dependent). The card
>   on this bench presents Wi-Fi via `brcmfmac` (Broadcom, station interface `wlp1s0`,
>   PCIe-attached) and Bluetooth via `btnxpuart` (NXP, UART-attached, `hci0`).
> - **uuu flash needs five artifacts** — wic, `imx-boot-imx8mq-evk`, a separate no-HDMI SPL
>   variant `imx-boot-imx8mq-evk-nohdmi` (for the redundant slot), `u-boot-imx8mq-evk.itb`,
>   and a `sit.bin` (Secondary Image Table) — see §0.4.
> - **Boot mode** is set via the `SW802` (boot mode, 2-pole) and `SW801` (boot device, 4-pole)
>   DIP switches (§0.1). The debug-UART connector is a Micro-USB (a CP2105 dual-UART bridge);
>   the A53 console is on CP2105 interface `00` — the `ttyUSB` index itself drifts (§0.2).
...
## Boot chain (imx8mq-evk, FoundriesFactory LmP)
...
| 2. **SPL** + DDR firmware | `imx-boot-<machine>` (e.g. `imx-boot-imx8mq-evk`); flashed by uuu into eMMC partitions `bootloader` (slot A) and `bootloader_s` (slot B). On the i.MX 8M Quad, slot B gets the **no-HDMI** build `imx-boot-imx8mq-evk-nohdmi` instead of an identical copy (see §0.4). | BootROM (AHAB signature) | SRK fuses |
| 3. **fit-image** — ATF (BL31) + OP-TEE (BL32) + U-Boot proper (BL33) + boot script | published as a *separate* artifact `u-boot-<machine>.itb` (e.g. `u-boot-imx8mq-evk.itb`); flashed by uuu into eMMC partitions `bootloader2` (slot A) and `bootloader2_s` (slot B). The flash also writes `sit-imx8mq-evk.bin` (a 512-byte Secondary Image Table) to the `sit` partition — it tells the BootROM where the bootloader images live. | LmP-patched SPL via FIT signature check | **Factory-specific public key embedded in SPL's devicetree at build time** |
...
**When Claude executes this plan, it should call `AskUserQuestion` at the start to collect all variables below** rather than hardcoding values. Suggested defaults reflect a typical lab bench but the plan should work for any Factory whose i.MX 8M Quad EVK target uses the `imx8mq-evk` MACHINE.

| Variable | Suggested default | Notes |
|---|---|---|
| `$FACTORY` | `nxp-imx` | the Factory the device registers to |
| `$DEVICE` | `imx8mq-evk-dev` | a fresh device name; bump on re-runs (registration is one-shot) |
...
| `$SERIAL_DEV` | the CP2105 interface-`00` node (often `/dev/ttyUSB0`) | A53 debug UART. The debug-UART bridge is a CP2105 dual-UART → two `ttyUSB` nodes; the A53 Linux console is **CP2105 interface `00`**, the other UART is silent. The `ttyUSB` index drifts as boards are swapped — resolve it per §0.3, don't hardcode. |
...
- imx8mq-evk hardware on the bench, powered via its **12 V / 5 A barrel-jack**, Ethernet plugged into the lab LAN, debug-UART cable (to the EVK's debug connector — see §0.2) to the host
- USB cable available for the `Download` / OTG port (for uuu flashing in §0)
- Boot-mode switch positions known — `SW802` / `SW801`, see the §0.1 table (cross-check against the board silkscreen)
...
## Phase 0 — Lab setup (imx8mq-evk)

### 0.1 Boot-mode switch cheat sheet

The imx8mq-evk selects its boot device with two boot-mode DIP switches printed on the PCB:

- **`SW802`** — the **boot-mode** switch, **2 poles** (selects internal boot vs serial downloader).
- **`SW801`** — the **boot-device** switch, **4 poles** (selects eMMC vs microSD when in internal boot).

The board silkscreen — printed on the PCB next to those switches — documents the settings.
The table below is from the NXP i.MX 8M Quad EVK Hardware User's Guide and the FoundriesFactory
reference manual; cross-check it against the silkscreen on *your* board, as the exact bit
patterns can be board-revision-specific.

| Mode | `SW802` (pole 1,2) | `SW801` (pole 1,2,3,4) | When to use |
|---|---|---|---|
| eMMC (default for this plan) | `ON, OFF` (internal boot) | `OFF, OFF, ON, OFF` | all OTA / rollback / app phases |
| USB — SDP (Serial Download Protocol) | `OFF, ON` (serial downloader) | don't-care (leave at the eMMC pattern) | first-time provisioning (§0.4) and uuu recovery |
| microSD | `ON, OFF` (internal boot) | `ON, ON, OFF, OFF` | only if deliberately testing an SD-card boot variant |

Always power the board off before changing the boot-mode switches. Record the current setting before changing — restoring it is a per-test ritual.

**Diagnostic clue — hostname suffix:** if the booted system reports its hostname as `<machine>-sd` (e.g. `imx8mq-evk-sd`) instead of `<machine>` (e.g. `imx8mq-evk`), the board booted from SD, *not* eMMC. The `-sd` suffix is not a build-flavor naming convention — it's a literal indicator that the SD-card image is what loaded. If you just flashed eMMC and you're seeing `-sd` in the hostname or login banner, the boot-mode switches are wrong: power off, set them to the eMMC position (per the EVK silkscreen — see above), power on. (The `-sd` can read like an LmP build-variant suffix — it isn't; it's a literal boot-source tag.)

### 0.2 Cabling

Board orientation for the descriptions below: hold the EVK so the **SoM text reads normally**.

Cross-check the silkscreen labels and connector placement below against your EVK revision:

| Cable | Board port (silkscreen label) | Host side | Purpose |
|---|---|---|---|
| USB debug | the **Micro-USB** debug-UART connector — on the **right edge** of the board when the silkscreen "NXP" word reads right-side up (the printed label is hard to read) | a **Silicon Labs CP2105 dual-UART bridge** → two `/dev/ttyUSB*` nodes (index drifts — see §0.3) | **A53 Linux console = CP2105 interface `00`**; interface `01` is silent |
| USB OTG | the `Download` / OTG port — a **USB-C** connector next to the power barrel-jack (the printed label is hard to read) | host USB-C / Type-A | uuu downloads in SDP mode |
| Ethernet | RJ-45 (`end0`) | lab switch | DHCP for `$DEVICE_IP` and SSH/OTA |
| Power | the EVK's **12 V / 5 A barrel-jack** | 12 V / 5 A DC supply | required even with the `Download` port attached |

### 0.3 Open the serial console with logging

The imx8mq-evk debug-UART is a **Silicon Labs CP2105 dual-UART bridge** — it presents
**two** `/dev/ttyUSB*` nodes, and the index numbering is **not stable**: it shifts as
boards are unplugged/replugged. The **A53 Linux console is on CP2105 interface `00`**;
interface `01` (the bridge's other UART) is silent on this board. Identify the node by
interface number, not a fixed index:

```bash
# list each ttyUSB with its CP2105 interface number — A53 console = interface 00
for d in /dev/ttyUSB*; do
  echo "$d -> iface $(udevadm info -q property -n "$d" | sed -n 's/^ID_USB_INTERFACE_NUM=//p')"
done
```

If unsure, probe: `stty -F <node> 115200 raw -echo`, send a CR, and the A53 console
echoes the `imx8mq-evk-… login:` banner; the other UART stays silent.
...
A53_TTY=/dev/ttyUSB0          # the interface-00 node from the step above — re-check, it drifts
screen -L -Logfile "$SERIAL_LOG" "$A53_TTY" 115200
...
If `screen` exits but the log keeps having stale data, it's because another `screen`/`picocom` already owns the tty — `fuser /dev/ttyUSB1` to find it.
...
# Five separate artifacts — all required. The i.MX 8M Quad flash includes a sit.bin AND a
# separate no-HDMI imx-boot variant (imx-boot-imx8mq-evk-nohdmi) for the redundant slot.
fioctl targets artifacts -f $FACTORY $TARGET \
  imx8mq-evk/lmp-factory-image-imx8mq-evk.wic.gz > lmp-factory-image-imx8mq-evk.wic.gz
fioctl targets artifacts -f $FACTORY $TARGET \
  imx8mq-evk/imx-boot-imx8mq-evk > imx-boot-imx8mq-evk
fioctl targets artifacts -f $FACTORY $TARGET \
  imx8mq-evk/imx-boot-imx8mq-evk-nohdmi > imx-boot-imx8mq-evk-nohdmi
fioctl targets artifacts -f $FACTORY $TARGET \
  imx8mq-evk/u-boot-imx8mq-evk.itb > u-boot-imx8mq-evk.itb
fioctl targets artifacts -f $FACTORY $TARGET \
  imx8mq-evk/sit-imx8mq-evk.bin > sit-imx8mq-evk.bin
fioctl targets artifacts -f $FACTORY $TARGET \
  imx8mq-evk-mfgtools/mfgtool-files-imx8mq-evk.tar.gz > mfgtool-files.tar.gz

tar xzf mfgtool-files.tar.gz              # creates mfgtool-files-imx8mq-evk/ with uuu, full_image.uuu, bundled mfgtool SPL/fit
# do NOT gunzip the wic — uuu's `flash -raw2sparse` reads .wic.gz directly
```

Two non-obvious things to know about these artifacts:

- **The wic is `lmp-factory-image-…`** — FoundriesFactory CI on i.MX targets publishes the factory-image variant (which includes Foundries-specific extras), not `lmp-base-console-image-…`. Adjust the artifact name if your Factory's CI publishes a different variant; check via `fioctl targets artifacts -f $FACTORY $TARGET | grep '^imx8mq-evk/' | grep wic`.
- **The `mfgtool-files` tarball is published under a *separate* build run** named `<machine>-mfgtools/`, *not* under the main machine path. So the mfg artifact path is `imx8mq-evk-mfgtools/mfgtool-files-imx8mq-evk.tar.gz` — note the `-mfgtools` suffix on the run name. uuu, `full_image.uuu`, the bootstrap mfgtool SPL (`imx-boot-mfgtool`), and a bootstrap fit-image (`u-boot-mfgtool.itb`) all live inside this tarball.

What `full_image.uuu` actually does:

```
SDP:  boot -f imx-boot-mfgtool                          # SDP-loads bundled mfgtool SPL + DDR fw
SDPS: boot -f imx-boot-mfgtool                          # SDPS variant — uuu uses this instead if the BootROM supports stream mode
SDPV: delay 1000; write -f u-boot-mfgtool.itb; jump     # SDPV (plugin) stage loads the mfgtool fit-image
...
FB: flash -raw2sparse all ../lmp-factory-image-imx8mq-evk.wic.gz/*
FB: flash bootloader     ../imx-boot-imx8mq-evk          # SPL → eMMC "bootloader" partition (slot A)
FB: flash bootloader2    ../u-boot-imx8mq-evk.itb        # fit-image → "bootloader2" partition (slot A)
FB: flash bootloader_s   ../imx-boot-imx8mq-evk-nohdmi   # SPL → "bootloader_s" (slot B) — NO-HDMI variant (see note below)
FB: flash bootloader2_s  ../u-boot-imx8mq-evk.itb        # fit-image → "bootloader2_s" (slot B)
FB: flash sit            ../sit-imx8mq-evk.bin           # Secondary Image Table → "sit" partition
FB: ucmd mmc partconf ${emmc_dev} ${emmc_ack} 1 0
FB: done
```

Maps directly to the boot architecture: SPL goes to `bootloader{,_s}`, fit-image goes to `bootloader2{,_s}`, two slots each so fiovb has somewhere to fall back to. **i.MX 8M Quad quirk — the redundant SPL slot gets a different image:** `bootloader_s` is flashed with `imx-boot-imx8mq-evk-nohdmi`, a build of `imx-boot` *without* the HDMI firmware blob that the primary `imx-boot-imx8mq-evk` carries (`# secondary boot image should not contain HDMI FW`). So you must pull `imx-boot-imx8mq-evk-nohdmi` as its own artifact alongside the primary. The flash also writes a **`sit.bin`** — `sit-imx8mq-evk.bin`, a 512-byte Secondary Image Table that tells the BootROM where the bootloader images live, published as the *separate artifact* `imx8mq-evk/sit-imx8mq-evk.bin`. That makes **five** separate artifacts (wic, `imx-boot-imx8mq-evk`, `imx-boot-imx8mq-evk-nohdmi`, `u-boot-imx8mq-evk.itb`, `sit-imx8mq-evk.bin`) plus the mfgtool-files tarball. uuu offers the bootstrap SPL over both `SDP:` and `SDPS:` — the BootROM picks whichever it supports — then `SDPV:` loads the mfgtool fit-image; the two-stage HID Serial Download protocol.

Power off → set the boot-mode switches to the USB-SDP position (`SW802` = `OFF, ON` — see the §0.1 table) → power on → connect a USB cable from the host to the **`Download`** / OTG port. Confirm the host sees the SDP device:

```bash
lsusb | grep -iE 'NXP|Freescale'
# Expected: "1fc9:0152 NXP Semiconductors USB download gadget" — the i.MX 8M Quad SDP USB ID
# (HID-based Serial Download protocol). Some i.MX8MQ silicon revisions instead enumerate as
# 1fc9:012b — uuu's built-in device table recognises both.
```

Run uuu using the binary bundled in the tarball (do *not* use a system-installed uuu unless you've verified the version matches). **Invoke from the parent `uuu-target<N>/` directory, not from inside the mfgtool subdir** — the `.uuu` script's `../<artifact>` paths resolve relative to where you run uuu from, and you want them to land in the parent dir where you placed the five artifacts:

```bash
# from uuu-target<N>/:
mfgtool-files-imx8mq-evk/uuu mfgtool-files-imx8mq-evk/full_image.uuu \
  2>&1 | tee uuu-flash-$(date -u +%Y%m%dT%H%M%SZ).log
# ~2 min; SDP-boots mfgtool SPL → fastboot → 6 flash ops to eMMC (raw2sparse rootfs + bootloader,
# bootloader2, bootloader_s, bootloader2_s, sit).
...
Power off → restore the boot-mode switches to the eMMC position (`SW802` = `ON, OFF`, `SW801` = `OFF, OFF, ON, OFF` — see the §0.1 table) → power on → watch `$SERIAL_LOG`. Expect:
...
```bash
fio@imx8mq-evk:~$ ip -4 addr show eth0 | awk '/inet /{print $2}' | cut -d/ -f1
192.168.1.42
```
...
|---|---|---|
| `bootloader`     | SPL + DDR fw (`imx-boot-imx8mq-evk`) | slot A — primary |
| `bootloader_s`   | SPL + DDR fw, **no-HDMI variant** (`imx-boot-imx8mq-evk-nohdmi`) | slot B — fallback for SPL |
| `bootloader2`    | fit-image (`u-boot-imx8mq-evk.itb`) | slot A — primary fit-image |
| `bootloader2_s`  | identical copy of `bootloader2` | slot B — fallback for fit-image |
| `sit`            | Secondary Image Table (`sit-imx8mq-evk.bin`) | BootROM image-offset index |

Note `bootloader_s` is **not** an identical copy of `bootloader` on the i.MX 8M Quad — the slot-B SPL is the `imx-boot-imx8mq-evk-nohdmi` build, which omits the HDMI firmware blob (see §0.4). The `bootloader2` slots *are* identical copies of each other.
...
Record the four sha256s, the value of `fiovb.is_secondary_boot`, and `fiovb.bootfirmware_version` as the **`GOOD_FIT`** baseline so §1.5 can confirm they changed across the OTA. The two `bootloader2` (fit-image) slots start identical after the uuu flash; the first OTA is what creates the asymmetry. Note the two **`bootloader`** (SPL) slots do **not** start identical on the i.MX 8M Quad — slot B is the no-HDMI build (`imx-boot-imx8mq-evk-nohdmi`), so `bootloader` and `bootloader_s` have different sha256s from the very first flash (see §0.4 / §0.7).
...
| 1.1.2 | host | `fioctl devices show $DEVICE -f $FACTORY` | current target (expect `imx8mq-evk-lmp-1`), tag (`main`), online state |
...
`fioctl targets tag` triggers a short CI metadata-update job; the new tag is live in the TUF metadata once that job finishes (~1-2 min). Verify with `fioctl targets show -f $FACTORY <N> | grep -A2 'Target: imx8mq-evk-lmp-<N>'` — the `Tags:` line should list both `main` and `fio-test-plan`.
...
- `interface-test-imx8mq-evk.sh` available in this same directory — the executable interface test (23 §-sections, numbered §0–§22), already adapted to imx8mq hardware
...
`interface-test-imx8mq-evk.sh` is a self-contained device-side script — it runs all "always-on baseline" rows and skips destructive/optional ones (USB/SD with media, RTC persistence, memtester, WDT trigger, thermal-load).
...
**Step 1 — full results → `interface-test-results-imx8mq-<datetime>.md`.** Pipe the script to the device over SSH and capture its GitHub-flavored-markdown output (one table per §-section, plus the script's own PASS/FAIL/INFO/SKIP totals block):

```bash
TS=$(date -u +%Y%m%dT%H%M%SZ)
sshpass -p fio ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o LogLevel=ERROR \
  fio@$DEVICE_IP 'bash -s' < interface-test-imx8mq-evk.sh > interface-test-results-imx8mq-$TS.md
```

**Step 2 — high-level summary → `interface-test-summary-imx8mq-<datetime>.md`.** A one-row-per-§-section (per-interface) roll-up — the at-a-glance verdict view — derived from the Step-1 results. A section's verdict is FAIL if any of its rows FAILed, else PASS; the **Notes** column explains any FAIL by listing the failing check(s):

```bash
{
  printf '# Interface test — high-level summary by interface\n\n'
  printf '**Detail:** `interface-test-results-imx8mq-%s.md`\n\n' "$TS"
  ...
  awk -F'|' '
    ...
    END           { emit() }
  ' interface-test-results-imx8mq-$TS.md
} > interface-test-summary-imx8mq-$TS.md
```
...
| §10.x i.MX-specific | PASS | `fw_printenv \| grep ^fiovb\\.` returns the 9 expected vars; `cat /sys/devices/soc0/soc_id` matches `i.MX8MQ`; CAAM RNG present at `/dev/hwrng` |
| §11.x USB (optional) | PASS for always-on baseline; rest skipped unless a device is plugged in | imx8mq USB is **DWC3 USB-3.0 (xHCI)** — `dwc3` controllers driven by `xhci-hcd` (*not* the i.MX 8M Mini's ChipIdea `ci_hdrc` USB-2.0-only controllers). Always-on: ≥1 `dwc3` controller bound under `/sys/bus/platform/drivers/dwc3/`; an xHCI host under `/sys/bus/platform/drivers/xhci-hcd/`; a UDC entry in `/sys/class/udc/` (`38100000.usb`). The two DWC3 controllers are devicetree nodes `usb@38100000` / `usb@38200000`, bound as `38100000.usb` / `38200000.usb`. Root hubs: `lsusb` should show both a USB-2.0 (`1d6b:0002`) and a USB-3.0 (`1d6b:0003`) root hub. Root-hub count is **INFO** — `0` is fine if no port is in host mode. With a USB stick on a host-mode port: device enumerates and mounts. |
| §12.x microSD slot + eMMC health | PASS for always-on baseline; rest skipped unless a card is inserted | imx8mq usdhc mmc hosts (same base addresses as the i.MX 8M Mini): `mmc0` = `30b40000.mmc` (usdhc1, eMMC, non-removable), `mmc1` = `30b50000.mmc` (usdhc2, the microSD slot, has `cd-gpios`). Always-on (no card): the SD-slot `mmc` host present + bound to its `usdhc` node, no `/dev/mmcblk1*`. Card-detect-GPIO journal trace is INFO (not all revs log it). With a microSD inserted: `/dev/mmcblk1` appears, partitions mount. **eMMC health** (always-on baseline): detects the eMMC (the `mmcblk` whose sysfs `device/type`=MMC) → PASS; `pre_eol_info` → PASS at `0x01` Normal, FAIL at `0x02` Warning / `0x03` Urgent; `life_time` reported INFO — all read from sysfs, no `mmc-utils` in the image required. |
| §13.x RTC (optional) | PASS for always-on baseline; persistence + alarm checks skipped unless explicitly run | Always-on (no special hw): `/dev/rtc0` present, `/sys/class/rtc/rtc0/name = snvs_rtc 30370000.snvs:snvs-rtc-lp` (i.MX 8M Quad internal SNVS RTC), `hwclock --show` returns a parseable time matching system clock to within a second. **Expected baseline gotcha:** `/sys/class/rtc/rtc0/hctosys=0` and dmesg shows the boot RTC read returned epoch (1970-01-01) — this is normal on dev EVKs without the SNVS coin-cell battery installed; NTP brings the time back. Optional persistence test (cold-boot, see if RTC time survives) only PASSes if a battery is fitted. |
| §14.x memory | PASS for always-on baseline; stress test optional | Always-on (no extras): `MemTotal` ≥ 2,700,000 kB (the imx8mq-evk's **3 GiB** LPDDR4 minus firmware/CMA reservations reads **2,957,640 kB**), `CmaTotal` = **917,504 kB / 896 MiB** (the i.MX 8M Mini used 655,360 kB / 640 MiB), `MemAvailable / MemTotal` > 0.5 at idle (~0.83), `SwapTotal` non-zero (zram ≈ 2,888,320 kB; see §1.x), no `out of memory` / `oom-kill` / `killed process` in the boot journal. Expected absences (don't false-FAIL): EDAC counters (non-ECC LPDDR4), `/proc/pressure/memory` (kernel may not have CONFIG_PSI). |
| §15.x QSPI/FlexSPI | NOT TESTED — expected-absence row | This build's devicetree leaves the i.MX 8M Quad QSPI/FlexSPI controller disabled: no `qspi`/`flexspi` platform device is instantiated and the `nxp-fspi` driver is present but unbound, so no NOR flash is visible to the kernel. Verdict is informational: the row documents what's *currently* off so a future build that enables it causes a visible verdict flip (from "absent" to either PASS-with-MTD-checks or FAIL-controller-bound-but-no-chip). Until then, intentionally not exercised. |
| §16.x HW watchdog | PASS for always-on baseline; trigger test optional (destructive) | Always-on (no special hw): `/dev/watchdog0` present, `wdctl -O /dev/watchdog0` returns `IDENTITY="imx2+ watchdog" TIMEOUT="60"` (the `imx2-wdt` driver bound to the internal WDT at node `watchdog@30280000` → `30280000.watchdog`). systemd's `RebootWatchdogSec=60` means WDT is used as a *shutdown safety net*, but `RuntimeWatchdogUSec=0` — i.e. nothing kicks it during steady-state operation. Bootstatus flags all 0 (last reset was NOT WDT-induced). Optional trigger test resets the board on purpose. |
| §17.x temperature sensors | PASS for always-on baseline; load test optional | Always-on (no special hw): the i.MX 8M Quad TMU exposes **one** `cpu-thermal` zone (same as the i.MX 8M Mini). The zone reads 15°C ≤ T ≤ 85°C at idle. Trip points are INFO: passive `85°C` / critical `95°C`. Cooling device: `cooling_device0=cpufreq-cpu0`. |
| §18.x Wayland / display | PASS on `lmp-wayland` builds; whole section N/A on plain `lmp` | **Only applies when DISTRO is `lmp-wayland`** (target 10 onward). Display path: the i.MX 8M Quad EVK drives display through the **DCSS (Display Controller Subsystem)** plus a Cadence HDP / `cdns-mhdp` HDMI controller; the DRM connector is `card0-HDMI-A-1`. Always-on: `weston` service active + process running; `/dev/galcore` (Vivante GPU) present; `/dev/dri/card0` + `/dev/dri/renderD128` present; `seatd` **optional/INFO** (logind libseat backend); a **3D GPU render check** — `weston-simple-egl` is run against the live compositor for ~5 s under `timeout`, PASS if it creates an EGL/GLES context and renders without crashing (this exercises the GC7000Lite end-to-end, catching a UMD/kernel-driver mismatch or GPU hang that the node-presence checks miss; SKIP if `weston-simple-egl` is not in the image). Display-attached checks (connector status, mode, `fb0` size) are INFO unless a monitor is known to be plugged in. On a plain `lmp` build the entire row is N/A. |
| §19.x Wi-Fi radio | PASS for always-on baseline + scan | The imx8mq-evk Wi-Fi is on the M.2 slot (swappable, so chip/driver are module-dependent). The card on this bench is Broadcom: driver `brcmfmac`, station interface `wlp1s0` (PCIe-attached). Always-on: station interface present (`wl*`/`mlan*`), the Wi-Fi driver module loaded, `wpa_supplicant` active, `nmcli radio` shows WIFI `enabled`. Scan check: bring the station interface up → `nmcli device wifi rescan` → expect **≥1 AP** visible (proves the radio tunes and receives). `0` APs is a FAIL *unless* the bench is RF-isolated. Association → DHCP → traffic is the optional deeper test (needs AP credentials). |
| §20.x Bluetooth | PASS for always-on baseline + power-on/scan | The imx8mq-evk Bluetooth is on the M.2 slot (swappable). The card on this bench presents a UART-attached NXP controller — driver `btnxpuart`, `hci0` Bus: UART. Always-on: `hci0` present (`hciconfig`), `bluetooth.service` active + `bluetoothd` running, `hci0` not rfkill-blocked, `bluetoothctl show` enumerates the controller. Power-on/scan check: `bluetoothctl power on` → `hci0 UP` → timed `scan on` → expect **≥1 device** discovered. `0` devices is a FAIL *unless* the bench is RF-isolated. Pairing/connect is the optional deeper test. |
| §21.x audio | PASS for the always-on baseline; tone playback skipped | ALSA is up with the on-board cards — the i.MX 8M Quad EVK registers **4**: `imxspdif`, `imxhdmiarc`, `imxaudiohdmi`, `wm8524audio` (SPDIF, HDMI-ARC, HDMI-audio, and the WM8524 analog DAC). `aplay -l` / `arecord -l` enumerate playback + capture devices, and `/dev/snd` has the `controlC*` / `pcmC*` nodes. Optional audio-expansion / BT-SCO codec nodes may stay in deferred-probe — **expected absences** on a bare EVK, reported INFO not FAIL. Actual tone playback needs a speaker/headset — skipped. |
| §22.x PCIe | PASS only with a fitted card; otherwise INFO — never FAIL | The i.MX 8M Quad has two PCIe Gen2 controllers (DesignWare-based, driver `imx6q-pcie`). Always-on (no card): the script reports the `*.pcie` devicetree node(s), whether the host bridge bound, and the `/sys/bus/pci/devices` count — all **INFO**, since a PCIe link only comes up with an endpoint card fitted in a slot (a bare bench legitimately shows no link / "PHY link never came up"). It goes **PASS** only when an endpoint enumerates past the host bridge; the link+traffic test is **SKIP** (needs a card). |
...
The interface test exists as the executable **`interface-test-imx8mq-evk.sh`** (see §6.2) — 23
...
To extend or audit it, edit `interface-test-imx8mq-evk.sh` directly — each `header "§N …"` block
...
1. Power off → set the boot-mode switches to USB-SDP (`SW802` = `OFF, ON` — §0.1 table) → power on → connect a USB cable to the `Download` / OTG port
2. Run uuu against the latest known-good `full_image.uuu` (re-fetch from `fioctl targets artifacts` if the local copy is stale)
3. Power off → restore the boot-mode switches to eMMC (`SW802` = `ON, OFF`, `SW801` = `OFF, OFF, ON, OFF` — §0.1 table) → power on
...
- Cortex-M4 co-processor firmware (loaded by U-Boot, separate update mechanism)
...
- [NXP i.MX 8M Quad EVK product page](https://www.nxp.com/design/design-center/development-boards-and-designs/i-mx-evaluation-and-development-boards/evaluation-kit-for-the-i-mx-8m-applications-processor:MCIMX8M-EVK) — pinout, boot-mode switches, connector designators; consult the board user-guide matching your EVK revision
- [uuu (Universal Update Utility) on GitHub](https://github.com/nxp-imx/mfgtools)
- Interface test: `interface-test-imx8mq-evk.sh` (the executable Phase 6 sweep — see §6.2)
